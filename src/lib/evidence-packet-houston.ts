/**
 * Houston Evidence Packet Generator
 * Generates the evidence-only PDF for upload to HCAD (no filing instructions, no hearing script).
 * This is the "ammo" — comps, brief, property data.
 */

import { escapeHtml } from "@/lib/security";

export interface HoustonEvidenceData {
  acct: string;
  address: string;
  city: string;
  state: string;
...
export function generateHoustonEvidenceHtml(data: HoustonEvidenceData): string {
  ...
  <div class="page">
    ...
    <div class="header">
      <div class="header-left">
        <div style="display: flex; align-items: center; gap: 10px;">
          ...
          <div>
            ...
            <div class="subtitle">Evidence Packet — Upload This to ${escapeHtml(data.neighborhoodCode ? 'HCAD' : 'the Appraisal District')}</div>
          </div>
        </div>
      </div>
      <div class="header-right">
        ...
        <div>Filing Body: HCAD / ARB</div>
      </div>
    </div>

    <!-- Upload instruction -->
    <div class="upload-note">
      <strong>📎 This is your evidence packet.</strong> Upload this document when filing your protest on hcad.org. See the Quick Start Guide for step-by-step instructions.
    </div>
