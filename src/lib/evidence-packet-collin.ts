/**
 * Collin Evidence Packet Generator
 * Generates the evidence-only PDF for upload to CCAD (no filing instructions, no hearing script).
 * This is the "ammo" — comps, brief, property data.
 */

import { escapeHtml } from "@/lib/security";

export interface CollinEvidenceData {
  acct: string;
  address: string;
  city: string;
  state: string;
...
export function generateCollinEvidenceHtml(data: CollinEvidenceData): string {
  const today = new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" });

  // Build property details rows (Collin has rich data similar to Dallas/Austin)
  ...
  // Generate Collin-specific brief
  ...
  <div class="page">
    ...
    <div class="header">
      <div class="header-left">
        <div style="display: flex; align-items: center; gap: 10px;">
          ...
          <div>
            ...
            <div class="subtitle">Evidence Packet — Upload This to CCAD</div>
          </div>
        </div>
      </div>
      <div class="header-right">
        ...
        <div>Filing Body: CCAD / ARB</div>
      </div>
    </div>

    <!-- Upload instruction -->
    <div class="upload-note">
      <strong>📎 This is your evidence packet.</strong> Upload this document when filing your protest on collincad.org. See the Quick Start Guide for step-by-step instructions.
    </div>
