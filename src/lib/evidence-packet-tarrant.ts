/**
 * Tarrant Evidence Packet Generator
 * Generates the evidence-only PDF for upload to TAD (no filing instructions, no hearing script).
 * This is the "ammo" — comps, brief, property data.
 */

import { escapeHtml } from "@/lib/security";

export interface TarrantEvidenceData {
  acct: string;
  address: string;
  city: string;
  state: string;
...
export function generateTarrantEvidenceHtml(data: TarrantEvidenceData): string {
  ...
  // Generate Tarrant-specific brief
  const features: string[] = [];
  if (data.stories) features.push(`${escapeHtml(data.stories)} story`);
  if (data.qualityGrade) features.push(`${escapeHtml(data.qualityGrade)} quality grade`);
  if (data.condition) features.push(`${escapeHtml(data.condition)} condition`);
  ...
  <div class="page">
    ...
    <div class="header">
      <div class="header-left">
        <div style="display: flex; align-items: center; gap: 10px;">
          ...
          <div>
            ...
            <div class="subtitle">Evidence Packet — Upload This to TAD</div>
          </div>
        </div>
      </div>
      <div class="header-right">
        ...
        <div>Filing Body: TAD / ARB</div>
      </div>
    </div>

    <!-- Upload instruction -->
    <div class="upload-note">
      <strong>📎 This is your evidence packet.</strong> Upload this document when filing your protest on tad.org. See the Quick Start Guide for step-by-step instructions.
    </div>
    ...
    <div class="title-block">
      ...
      <div class="property-meta">
        ...
        ${data.qualityGrade ? `<span><strong>Quality Grade:</strong> ${escapeHtml(data.qualityGrade)}</span>` : ""}
        ${data.condition ? `<span><strong>Condition:</strong> ${escapeHtml(data.condition)}</span>` : ""}
      </div>
    </div>
    ...
    <div class="breakdown-row">
      <div class="breakdown-card">
        ...
        <table class="breakdown-table">
          ...
          ${data.qualityGrade ? `<tr><td>Quality Grade</td><td>${escapeHtml(data.qualityGrade)}</td></tr>` : ""}
          ${data.condition ? `<tr><td>Condition</td><td>${escapeHtml(data.condition)}</td></tr>` : ""}
