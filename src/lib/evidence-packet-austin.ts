/**
 * Austin Evidence Packet Generator
 * Generates the evidence-only PDF for upload to TCAD (no filing instructions, no hearing script).
 * This is the "ammo" — comps, brief, property data.
 */

import { escapeHtml } from "@/lib/security";

export interface AustinEvidenceData {
  acct: string;
  address: string;
  city: string;
  state: string;
...
export function generateAustinEvidenceHtml(data: AustinEvidenceData): string {
  const today = new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" });

  // Build property details rows (Austin has rich data similar to Dallas)
  ...
  // Generate Austin-specific brief
  ...
  if (data.extWall) features.push(`${escapeHtml(data.extWall)} exterior`);
  ...
  <div class="page">
    ...
    <div class="header">
      <div class="header-left">
        <div style="display: flex; align-items: center; gap: 10px;">
          ...
          <div>
            ...
            <div class="subtitle">Evidence Packet — Upload This to TCAD</div>
          </div>
        </div>
      </div>
      <div class="header-right">
        ...
        <div>Filing Body: TCAD / ARB</div>
      </div>
    </div>

    <!-- Upload instruction -->
    <div class="upload-note">
      <strong>📎 This is your evidence packet.</strong> Upload this document when filing your protest on traviscad.org. See the Quick Start Guide for step-by-step instructions.
    </div>
    ...
    <div class="title-block">
      ...
      <div class="property-meta">
        ...
        ${data.fullBaths ? `<span><strong>Bathrooms:</strong> ${bathsStr}</span>` : ""}
      </div>
    </div>
    ...
    <div class="breakdown-row">
      <div class="breakdown-card">
        ...
        <table class="breakdown-table">
          ...
          ${data.fullBaths ? `<tr><td>Bathrooms</td><td>${bathsStr}</td></tr>` : ""}
