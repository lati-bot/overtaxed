/**
 * Denton Evidence Packet Generator
 * Generates the evidence-only PDF for upload to Denton CAD (no filing instructions, no hearing script).
 * This is the "ammo" — comps, brief, property data.
 */

import { escapeHtml } from "@/lib/security";

export interface DentonEvidenceData {
  acct: string;
  address: string;
  city: string;
  state: string;
...
export function generateDentonEvidenceHtml(data: DentonEvidenceData): string {
  ...
  <div class="page">
    ...
    <div class="header">
      <div class="header-left">
        <div style="display: flex; align-items: center; gap: 10px;">
          ...
          <div>
            ...
            <div class="subtitle">Evidence Packet — Upload This to Denton CAD</div>
          </div>
        </div>
      </div>
      <div class="header-right">
        ...
        <div>Filing Body: Denton CAD / ARB</div>
      </div>
    </div>

    <!-- Upload instruction -->
    <div class="upload-note">
      <strong>📎 This is your evidence packet.</strong> Upload this document when filing your protest on dentoncad.com. See the Quick Start Guide for step-by-step instructions.
    </div>
    ...
    <div class="section">
      ...
      <div class="brief">
        <p>The subject property at <strong>${escapeHtml(data.address)}</strong> (Denton CAD Account: ${escapeHtml(data.acct)}), located in ${escapeHtml(data.city)}, TX, appraisal neighborhood ${escapeHtml(data.neighborhoodCode)}, is currently appraised at <strong>$${data.currentAssessment.toLocaleString()}</strong>, which equates to <strong>$${data.perSqft.toFixed(2)} per square foot</strong> of building area. The property is a ${data.sqft.toLocaleString()} sq ft residence${data.yearBuilt ? ` built in ${data.yearBuilt}` : ""}.</p>
