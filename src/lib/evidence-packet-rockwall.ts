/**
 * Rockwall Evidence Packet Generator
 * Generates the evidence-only PDF for upload to RCAD (no filing instructions, no hearing script).
 * This is the "ammo" — comps, brief, property data.
 * Note: Rockwall uses total value comparison, not $/sqft.
 */

import { escapeHtml } from "@/lib/security";

export interface RockwallEvidenceData {
  acct: string;
  address: string;
  city: string;
  state: string;
...
export function generateRockwallEvidenceHtml(data: RockwallEvidenceData): string {
  const today = new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" });

  // Comps table uses appraised value (Rockwall compares total values, not $/sqft)
  ...
  <div class="page">
    ...
    <div class="header">
      <div class="header-left">
        <div style="display: flex; align-items: center; gap: 10px;">
          ...
          <div>
            ...
            <div class="subtitle">Evidence Packet — Upload This to RCAD</div>
          </div>
        </div>
      </div>
      <div class="header-right">
        ...
        <div>Filing Body: RCAD / ARB</div>
      </div>
    </div>

    <!-- Upload instruction -->
    <div class="upload-note">
      <strong>📎 This is your evidence packet.</strong> Upload this document when filing your protest on rockwallcad.com. See the Quick Start Guide for step-by-step instructions.
    </div>
    ...
    <div class="title-block">
      ...
      <div class="property-meta">
        ...
        ${data.beds ? `<span><strong>Bedrooms:</strong> ${data.beds}</span>` : ""}
        ${data.baths ? `<span><strong>Bathrooms:</strong> ${data.baths}</span>` : ""}
      </div>
    </div>
    ...
    <div class="summary-row">
      <div class="summary-card alert">
        ...
        <div class="summary-detail">${data.overAssessedPct}% above comparable median</div>
      </div>
      <div class="summary-card success">
        ...
        <div class="summary-detail">Median of ${data.comps.length} comparable properties</div>
      </div>
      <div class="summary-card success">
    ...
    <div class="section">
      ...
      <div class="brief">
        <p>The subject property at <strong>${escapeHtml(data.address)}</strong> (RCAD Account: ${escapeHtml(data.acct)}), located in ${escapeHtml(data.city)}, TX, appraisal neighborhood ${escapeHtml(data.neighborhoodCode)}, is currently appraised at <strong>$${data.currentAssessment.toLocaleString()}</strong>. The property${data.beds ? ` has ${data.beds} bedrooms` : ""}${data.baths ? ` and ${data.baths} bathrooms` : ""}${data.yearBuilt ? ` and was built in ${data.yearBuilt}` : ""}.</p>
        
        <p>An analysis of ${data.comps.length} comparable properties within the same appraisal neighborhood demonstrates that the subject property is appraised at a value <strong>${data.overAssessedPct}% above the comparable median</strong> of $${data.compMedianValue.toLocaleString()}. Under <strong>Texas Tax Code §41.41(a)(2)</strong>, a property owner has the right to protest the appraised value on grounds that the property is unequally appraised. Under <strong>§42.26(a)</strong>, the appraised value of property must not exceed the median appraised value of a reasonable number of comparable properties appropriately adjusted.</p>
