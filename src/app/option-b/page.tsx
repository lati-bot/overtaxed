...
/* ═══════════════════════════════════════════════
   OPTION B — Dark/Charcoal CTA, Left-Aligned Hero
   Oura-inspired editorial. Quiet confidence.
   ═══════════════════════════════════════════════ */
...
export default function OptionB() {
  ...
  return (
    <div className="min-h-screen bg-[#f7f6f3] text-[#1a1a1a]" style={{ fontFamily: "'Inter', system-ui, -apple-system, sans-serif" }}>
        ...
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-8">
            <div className="hidden md:flex items-center gap-8 text-[13px] text-[#666] tracking-wide">
              <button onClick={() => scrollToSection("how-it-works-b")} className="hover:text-[#1a1a1a] transition-colors">How it Works</button>
              <button onClick={() => scrollToSection("pricing-b")} className="hover:text-[#1a1a1a] transition-colors">Pricing</button>
              <button onClick={() => scrollToSection("faq-b")} className="hover:text-[#1a1a1a] transition-colors">FAQ</button>
            </div>
            <button 
              onClick={() => scrollToSection("hero-b")}
      ...
      {/* Hero — LEFT-ALIGNED, charcoal CTA */}
      <section className="pt-20 sm:pt-28 pb-8 sm:pb-12 px-6" id="hero-b">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <div className="relative flex-1">
                  <input
                    ...
                    className="w-full h-14 px-5 rounded-full text-base bg-white border border-black/[0.08] text-[#1a1a1a] placeholder-[#aaa] focus:border-[#1a1a1a]/20 focus:outline-none focus:ring-2 focus:ring-[#1a1a1a]/5 transition-all"
                    value={address}
                      ...
                      {suggestions.map((s, i) => (
                        <button key={s.pin || s.acct || i} type="button"
                          ...
                          <div className="flex items-center justify-between gap-3">
                            ...
                            <span className="bg-[#f0f0ee] text-[#666] text-[11px] font-medium rounded-md px-2 py-0.5 whitespace-nowrap flex-shrink-0">
                ...
                <button type="submit" disabled={loading || !address.trim()}
                  className="h-14 px-8 rounded-full font-medium text-base transition-all disabled:opacity-40 bg-[#1a1a1a] text-white hover:bg-[#333] whitespace-nowrap">
                  ...
                  <>
                    ...
                    <form onSubmit={handleNotifySubmit} className="flex gap-2">
                      <input type="email" placeholder="you@email.com" value={notifyEmail} onChange={(e) => setNotifyEmail(e.target.value)} required
                        className="flex-1 h-11 px-4 rounded-full text-sm bg-[#f7f6f3] border border-black/[0.06] text-[#1a1a1a] placeholder-[#aaa] focus:outline-none focus:ring-2 focus:ring-black/5" />
                      <button type="submit" disabled={notifyLoading}
                        className="h-11 px-5 rounded-full text-sm font-medium bg-[#1a1a1a] text-white hover:bg-[#333] transition-colors disabled:opacity-50">
      ...
      {/* Stats — left-aligned with divider */}
      ...
      {/* How it works */}
      <section id="how-it-works-b" className="py-14 sm:py-20 px-6">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-12 sm:gap-16">
            {[
              ...
              <div key={step.num}>
                <div className="text-[14px] font-medium tracking-[0.15em] text-[#ccc] mb-4">{step.num}</div>
      ...
      {/* Pricing */}
      <section id="pricing-b" className="py-14 sm:py-20 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="rounded-2xl p-8 sm:p-12 bg-white border border-black/[0.06]">
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6 mb-8">
              ...
              <button onClick={() => { const el = document.getElementById("hero-b"); if (el) { el.scrollIntoView({ behavior: "smooth" }); setTimeout(() => inputRef.current?.focus(), 500); } }}
                className="h-14 px-8 rounded-full font-medium text-base bg-[#1a1a1a] text-white hover:bg-[#333] transition-colors">
            ...
            <div className="border-t border-black/[0.06] pt-8 mb-8">
              ...
              <div className="flex items-center justify-center gap-8">
                <div className="text-center">
                  <div className="text-2xl sm:text-3xl font-semibold text-[#1a1a1a]">$49</div>
      ...
      {/* FAQ */}
      <section id="faq-b" className="py-14 sm:py-20 px-6">
      ...
      {/* Footer CTA — dark */}
      <section className="py-20 sm:py-28 px-6 bg-[#1a1a1a] text-center">
        <div className="max-w-2xl mx-auto">
          ...
          <p className="text-lg text-[#888] font-light mb-10">
            The average homeowner saves $1,136/year. At $49, that&apos;s a 23x return.
          </p>
          <button onClick={() => { const el = document.getElementById("hero-b"); if (el) { el.scrollIntoView({ behavior: "smooth" }); setTimeout(() => inputRef.current?.focus(), 500); } }}
            className="h-14 px-10 rounded-full font-medium text-base bg-white text-[#1a1a1a] hover:bg-[#f0f0ee] transition-colors">
            Check My Property
          </button>
        </div>
      </section>
