...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { resources: neighborhoodComps } = await container.items.query({
      query: `SELECT TOP 20 c.id, c.acct, c.address, c.city, c.sqft, c.year_built, c.beds, c.full_baths, c.half_baths,
    ...
    const enrichedComps = neighborhoodComps
      .map((c: any) => ({
        ...
        beds: c.beds || 0,
        fullBaths: c.full_baths || 0,
        halfBaths: c.half_baths || 0,
    ...
    return NextResponse.json({
      ...
      city: mainProp.city || "AUSTIN",
      status: mainProp.potential_reduction > 0 ? "over" : "fair",
      sqft: mainProp.sqft,
      yearBuilt: mainProp.year_built,
      beds: mainProp.beds,
      fullBaths: mainProp.full_baths,
      halfBaths: mainProp.half_baths,
      current_assessment: mainProp.current_assessment,
      fair_assessment: mainProp.fair_assessment,
      estimated_savings: mainProp.estimated_savings,
    ...
    console.error("[austin/comps] Error:", error);
