...
// ──────────────────────────────────────────────────────────────────
// Austin Property Data Types
// ──────────────────────────────────────────────────────────────────

interface AustinComp {
  acct: string;
  address: string;
  sqft: number;
  assessedVal: number;
...
async function getAustinPropertyData(acct: string): Promise<AustinPropertyData | null> {
  try {
    ...
    // Map comps
    const comps: AustinComp[] = (prop.comps || []).map((c: any) => ({
      acct: c.acct || "",
      address: c.address || "",
      sqft: c.sqft || 0,
    ...
    console.error("[getAustinPropertyData] Error:", error);
    return null;
  }
}

// ──────────────────────────────────────────────────────────────────
// PDF HTML Generation — Austin / Travis County
// ──────────────────────────────────────────────────────────────────

function generateAustinPdfHtml(data: AustinPropertyData): string {
  ...
  const brief = generateAustinBrief(data);
  ...
  <div class="page">
    ...
    <div class="header">
      ...
      <div class="header-right">
        ...
        <div>Travis County, Texas</div>
        <div>Protest Type: Uniform & Equal / Market Value</div>
        <div>Filing Body: TCAD / Appraisal Review Board</div>
      </div>
    </div>
    ...
    <div class="breakdown-row">
      <div class="breakdown-card">
        ...
        <table class="breakdown-table">
          ...
          <tr><td>Bathrooms</td><td>${escapeHtml(bathsStr)}</td></tr>
          ${data.stories ? `<tr><td>Stories</td><td>${escapeHtml(data.stories)}</td></tr>` : ""}
          ${data.extWall ? `<tr><td>Exterior</td><td>${escapeHtml(data.extWall)}</td></tr>` : ""}
    ...
    <div class="section page-break">
      <div class="section-title">📋 How to Protest — Travis County Step by Step</div>
      <p style="font-size: 10px; color: #555; margin-bottom: 14px; line-height: 1.6;">Texas law gives every property owner the right to protest their appraised value. Travis Central Appraisal District (TCAD) makes this easy — you can file online. <strong>No attorney or agent needed.</strong></p>
      
      <div class="info-box warning">
        <p style="margin: 0;"><strong>⏰ Deadline:</strong> TCAD protest season typically opens when appraisal notices are mailed (usually late March/early April). The deadline is typically May 15 or 30 days after your notice date, whichever is later. <strong>File as soon as you receive your notice for the best results.</strong> Verify your exact deadline at <a href="https://www.traviscad.org" class="step-link">traviscad.org</a>.</p>
      </div>
      ...
      <div class="steps">
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">TCAD mails appraisal notices in <strong>late March to early April</strong>. Your notice will show your property's appraised value for the current year. Your account number is <strong style="font-family: monospace; background: #f0f0f0; padding: 1px 4px; border-radius: 3px;">${escapeHtml(data.acct)}</strong>.</div>
          </div>
        </div>
        <div class="step">
          <div class="step-number">2</div>
          <div class="step-content">
            <div class="step-title">File Online via TCAD Portal</div>
            <div class="step-desc">
              Go to <a href="https://www.traviscad.org/portal" class="step-link">traviscad.org/portal</a> → Set up your account using your <strong>property owner ID</strong> and <strong>PIN</strong> from your appraisal notice → File your protest<br/>
              • Your <strong>Account Number</strong>: <strong style="font-family: monospace; background: #f0f0f0; padding: 1px 4px; border-radius: 3px;">${escapeHtml(data.acct)}</strong><br/>
        ...
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">
              After filing, TCAD may offer a <strong>settlement</strong> without a hearing.<br/>
        ...
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">
              If settlement doesn't work, you'll be scheduled for an <strong>Appraisal Review Board (ARB) hearing</strong>. Hearings run from <strong>May through July</strong>. You can attend <strong>in person or by phone</strong>.<br/>
      ...
      <div class="info-box info">
        <p style="margin: 0;"><strong>📬 Alternative Filing Methods:</strong> You can also file your protest by <strong>mail</strong> (PO Box 149012, Austin, TX 78714) or <strong>in person</strong> at the TCAD office (850 East Anderson Lane, Austin, TX 78752).</p>
      </div>
    </div>
    ...
    <div class="section">
      ...
      <div style="font-size: 10px; line-height: 1.7; color: #555; margin-top: 12px;">
        ...
        <p style="margin-bottom: 8px;"><strong>No Cost to File:</strong> Filing a protest with TCAD is completely free. There is no penalty for protesting — even if your value isn't reduced.</p>
        <p style="margin-bottom: 8px;"><strong>Homestead Exemption:</strong> If you haven't already, apply for a <strong>homestead exemption</strong> through TCAD. This is separate from your protest and can reduce your taxable value by $100,000 (effective 2024). Go to <a href="https://www.traviscad.org" class="step-link">traviscad.org</a> → "Exemptions" to apply.</p>
...
function generateAustinBrief(data: AustinPropertyData): string {
  // Include richer property description
  ...
  return `
    <p>The subject property at <strong>${escapeHtml(data.address)}</strong> (TCAD Account: ${escapeHtml(data.acct)}), located in ${escapeHtml(data.city)}, TX, appraisal neighborhood ${escapeHtml(data.neighborhoodCode)}, is currently appraised at <strong>$${data.currentAssessment.toLocaleString()}</strong>, which equates to <strong>$${data.perSqft.toFixed(2)} per square foot</strong> of building area.${featuresStr}</p>
...
async function sendAustinEmail(
  email: string,
  acct: string,
  pdfBuffer: Buffer,
  ...
  await getResend().emails.send({
    ...
    html: `
      <div style="background: #f7f6f3; padding: 32px 16px;">
        ...
        <p style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #666; margin-bottom: 24px; font-size: 15px;">Everything you need to protest your property tax for <strong>${escapeHtml(data.address)}</strong> with the Travis Central Appraisal District.</p>
        ...
        <ul style="font-size: 14px; color: #555; margin-bottom: 24px; padding-left: 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
          ...
          <li>Step-by-step TCAD Portal and ARB hearing instructions</li>
          <li>Hearing script — exactly what to say</li>
          <li>Settlement guidance — how to evaluate TCAD's offer</li>
        </ul>
...
// ──────────────────────────────────────────────────────────────────
// Token handling — austin:acct:sessionId format
// ──────────────────────────────────────────────────────────────────

function generateAccessToken(sessionId: string, acct: string): string {
  return _genToken(`austin:${acct}:${sessionId}`);
}
...
function verifyAccessToken(token: string): { acct: string; sessionId: string } | null {
  ...
  if (parts.length < 3 || parts[0] !== "austin") return null;
...
export async function GET(request: NextRequest) {
  ...
  if (accessToken) {
    ...
    const propertyData = await getAustinPropertyData(tokenData.acct);
    if (!propertyData) {
      return NextResponse.json({ error: "Property not found" }, { status: 404 });
    }
    return NextResponse.json({ success: true, property: propertyData, token: accessToken, jurisdiction: "austin" });
  }
  ...
  if (sessionId) {
    try {
      ...
      if (!clientRef || !clientRef.startsWith("austin:")) {
        return NextResponse.json({ error: "Not an Austin property" }, { status: 400 });
      }
      const acct = clientRef.replace("austin:", "");
      const propertyData = await getAustinPropertyData(acct);
      if (!propertyData) {
        return NextResponse.json({ error: "Property not found" }, { status: 404 });
      }
      const token = generateAccessToken(sessionId, acct);
      // Send email
      console.log(`[Austin] Email: ${email || "NONE"}, Acct: ${acct}, Session: ${sessionId}`);
      if (email) {
        try {
          console.log("[Austin] Generating PDF...");
          const pdfBuffer = await generatePdf(generateAustinPdfHtml(propertyData));
          console.log(`[Austin] PDF generated: ${pdfBuffer.length} bytes`);
          await sendAustinEmail(email, acct, pdfBuffer, propertyData, token);
        ...
        console.warn("[Austin] No email from Stripe session — skipping email send");
      }
      return NextResponse.json({ success: true, property: propertyData, token, email: email || null, jurisdiction: "austin" });
    } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  const html = generateAustinPdfHtml(propertyData);
