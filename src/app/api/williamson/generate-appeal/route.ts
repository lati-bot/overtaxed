...
// ──────────────────────────────────────────────────────────────────
// Williamson Property Data Types
// ──────────────────────────────────────────────────────────────────

interface WilliamsonComp {
  acct: string;
  address: string;
  sqft: number;
  assessedVal: number;
...
interface WilliamsonPropertyData {
  acct: string;
  address: string;
  city: string;
  state: string;
...
async function getWilliamsonPropertyData(acct: string): Promise<WilliamsonPropertyData | null> {
  try {
    ...
    // Map comps — Williamson has NO beds/baths/pool
    const comps: WilliamsonComp[] = (prop.comps || []).map((c: any) => ({
      acct: c.acct || "",
      address: c.address || "",
      sqft: c.sqft || 0,
    ...
    console.error("[getWilliamsonPropertyData] Error:", error);
    return null;
  }
}

// ──────────────────────────────────────────────────────────────────
// PDF HTML Generation — Williamson County / WCAD
// ──────────────────────────────────────────────────────────────────

function generateWilliamsonPdfHtml(data: WilliamsonPropertyData): string {
  ...
  const brief = generateWilliamsonBrief(data);
  ...
  <div class="page">
    ...
    <div class="header">
      ...
      <div class="header-right">
        ...
        <div>Williamson County, Texas</div>
        <div>Protest Type: Uniform & Equal / Market Value</div>
        <div>Filing Body: WCAD / Appraisal Review Board</div>
      </div>
    </div>
    ...
    <div class="breakdown-row">
      ...
      <div class="breakdown-card">
        ...
        <table class="breakdown-table">
          ...
          <tr><td>Per Square Foot</td><td>$${data.perSqft.toFixed(2)}</td></tr>
        </table>
    ...
    <div class="section page-break">
      <div class="section-title">📋 How to Protest — Williamson County Step by Step</div>
      <p style="font-size: 10px; color: #555; margin-bottom: 14px; line-height: 1.6;">Texas law gives every property owner the right to protest their appraised value. Williamson Central Appraisal District (WCAD) makes this easy — you can file online at wcad.org. <strong>No attorney or agent needed.</strong></p>
      
      <div class="info-box warning">
        <p style="margin: 0;"><strong>⏰ Deadline:</strong> WCAD protest season typically opens when appraisal notices are mailed (usually late March/early April). The deadline is typically May 15 or 30 days after your notice date, whichever is later. <strong>File as soon as you receive your notice for the best results.</strong> Verify your exact deadline at <a href="https://www.wcad.org" class="step-link">wcad.org</a>.</p>
      </div>
      ...
      <div class="steps">
        ...
        <div class="step">
          ...
          <div class="step-content">
            <div class="step-title">Check for a Settlement Offer (Informal Review)</div>
            <div class="step-desc">
              After filing, WCAD may offer a <strong>settlement</strong> through their informal review process.<br/>
        ...
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">
              If settlement doesn't work, you'll be scheduled for a <strong>Williamson County Appraisal Review Board (ARB) hearing</strong>. Hearings run from <strong>May through July</strong>. You can attend <strong>in person or by phone</strong>.<br/>
    ...
    <div class="section">
      ...
      <div style="font-size: 10px; line-height: 1.7; color: #555; margin-top: 12px;">
        ...
        <p style="margin-bottom: 8px;"><strong>No Cost to File:</strong> Filing a protest with WCAD is completely free. There is no penalty for protesting — even if your value isn't reduced.</p>
        <p style="margin-bottom: 8px;"><strong>Homestead Exemption:</strong> If you haven't already, apply for a <strong>homestead exemption</strong> through WCAD. This is separate from your protest and can reduce your taxable value by $100,000 (effective 2024). Go to <a href="https://www.wcad.org" class="step-link">wcad.org</a> → "Forms & Exemptions" to apply.</p>
...
function generateWilliamsonBrief(data: WilliamsonPropertyData): string {
  const yearInfo = data.yearBuilt && data.yearBuilt > 0 
...
function buildQuickStartData(data: WilliamsonPropertyData): QuickStartData {
  return {
...
function buildCoverLetterData(data: WilliamsonPropertyData): CoverLetterData {
  return {
    ...
    state: "TX",
    filingBody: "WCAD",
    filingBodyFull: "Williamson Central Appraisal District",
    currentAssessment: data.currentAssessment,
    fairAssessment: data.fairAssessment,
    overAssessedPct: data.overAssessedPct,
    compCount: data.comps.length,
...
async function sendWilliamsonEmail(
  email: string,
  acct: string,
  quickStartPdf: Buffer,
  evidencePdf: Buffer,
  coverLetterPdf: Buffer,
  data: WilliamsonPropertyData,
  accessToken: string
): Promise<void> {
      ...
      <div style="background: #f7f6f3; padding: 32px 16px;">
        ...
        <p style="font-size: 14px; margin-bottom: 8px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;"><strong>Three PDFs attached — here's how to use them:</strong></p>
            ...
            <div style="background: #f0fdf4; border: 1px solid #1a6b5a; border-radius: 8px; padding: 14px 16px; margin-bottom: 16px;">
              ...
              <p style="margin: 0; font-size: 14px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;"><strong>📎 Evidence Packet</strong> — Upload this to WCAD when you file. It's your proof.</p>
            </div>
        <ul style="font-size: 14px; color: #555; margin-bottom: 24px; padding-left: 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
          ...
          <li>Step-by-step WCAD online protest instructions</li>
          <li>Hearing script — exactly what to say</li>
          <li>Settlement guidance — how to evaluate WCAD's offer</li>
        </ul>
...
// ──────────────────────────────────────────────────────────────────
// Token handling — williamson:acct:sessionId format
// ──────────────────────────────────────────────────────────────────
...
export async function GET(request: NextRequest) {
  ...
  if (accessToken) {
    ...
    const propertyData = await getWilliamsonPropertyData(tokenData.acct);
    if (!propertyData) {
      return NextResponse.json({ error: "Property not found" }, { status: 404 });
    }
    return NextResponse.json({ success: true, property: propertyData, token: accessToken, jurisdiction: "williamson" });
  }
  ...
  if (sessionId) {
    try {
      ...
      if (!clientRef || !clientRef.startsWith("williamson:")) {
        return NextResponse.json({ error: "Not a Williamson property" }, { status: 400 });
      }
      const acct = clientRef.replace("williamson:", "");
      const propertyData = await getWilliamsonPropertyData(acct);
      if (!propertyData) {
        return NextResponse.json({ error: "Property not found" }, { status: 404 });
      }
      const token = generateAccessToken(sessionId, acct);
      // Send email
      console.log(`[Williamson] Email: ${email || "NONE"}, Acct: ${acct}, Session: ${sessionId}`);
      if (email) {
        try {
          console.log("[Williamson] Generating PDFs...");
          ...
          const [quickStartPdf, evidencePdf, coverLetterPdf] = await Promise.all([
            ...
            generatePdf(generateWilliamsonEvidenceHtml(propertyData)),
            generatePdf(generateCoverLetterHtml(buildCoverLetterData(propertyData))),
          ]);
          console.log(`[Williamson] PDFs generated: quick-start ${quickStartPdf.length} bytes, evidence ${evidencePdf.length} bytes`);
          await sendWilliamsonEmail(email, acct, quickStartPdf, evidencePdf, coverLetterPdf, propertyData, token);
          console.log("[Williamson] Email sent successfully");
        } catch (emailErr) {
          console.error("[Williamson] Email send error:", emailErr);
          // Don't fail the response — email is best-effort
        }
      } else {
        console.warn("[Williamson] No email from Stripe session — skipping email send");
      }
      return NextResponse.json({ success: true, property: propertyData, token, email: email || null, jurisdiction: "williamson" });
    } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  const html = generateWilliamsonEvidenceHtml(propertyData);
