...
async function sendWilliamsonEmail(
  ...
  await getResend().emails.send({
    ...
    to: email,
    subject: `Property Tax Protest Package — ${data.address} — Williamson County`,
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; color: #1a1a2e;">
        ...
        <p style="color: #666; margin-bottom: 24px; font-size: 15px;">Everything you need to protest your property tax for <strong>${data.address}</strong> with the Williamson Central Appraisal District.</p>
...
function generateAccessToken(sessionId: string, acct: string): string {
  const data = `${sessionId}:williamson:${acct}:${Date.now()}`;
  const hash = crypto.createHash("sha256").update(data + process.env.STRIPE_SECRET_KEY).digest("hex");
  return `${Buffer.from(`williamson:${acct}:${sessionId}`).toString("base64url")}.${hash.slice(0, 16)}`;
}
...
export async function GET(request: NextRequest) {
  ...
  if (sessionId) {
    try {
      ...
      if (email) {
        try {
          console.log("[Williamson] Generating PDF...");
          const pdfBuffer = await generatePdf(generateWilliamsonPdfHtml(propertyData));
          console.log(`[Williamson] PDF generated: ${pdfBuffer.length} bytes`);
          await sendWilliamsonEmail(email, acct, pdfBuffer, propertyData, token);
...
export async function POST(request: NextRequest) {
  ...
  const html = generateWilliamsonPdfHtml(propertyData);
