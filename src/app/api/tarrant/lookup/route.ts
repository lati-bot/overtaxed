...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      console.error("[tarrant/lookup] Neighborhood stats error:", err);
    }

    // Format comps — Tarrant has quality_grade and condition instead of beds/baths
    const comps = (prop.comps || []).map((c: any) => ({
      ...
      perSqft: c.psf || 0,
      yearBuilt: c.yearBuilt || c.year_built || 0,
    ...
    return NextResponse.json({
      found: true,
      jurisdiction: "tarrant_county_tx",
      property: {
        acct: prop.acct,
    ...
    console.error("[tarrant/lookup] Error:", error);
