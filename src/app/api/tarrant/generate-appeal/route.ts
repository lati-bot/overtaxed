...
// ──────────────────────────────────────────────────────────────────
// Tarrant Property Data Types
// ──────────────────────────────────────────────────────────────────

interface TarrantComp {
  acct: string;
  address: string;
  sqft: number;
  assessedVal: number;
...
async function getTarrantPropertyData(acct: string): Promise<TarrantPropertyData | null> {
  try {
    ...
    // Map comps — Tarrant uses quality_grade and condition instead of beds/baths
    const comps: TarrantComp[] = (prop.comps || []).map((c: any) => ({
      acct: c.acct || "",
      address: c.address || "",
      sqft: c.sqft || 0,
      ...
      qualityGrade: c.quality_grade || undefined,
    ...
    // Enrich comps with quality_grade and condition from their own Cosmos documents
    const enrichedComps = await Promise.all(
      comps.map(async (comp) => {
        if (comp.qualityGrade && comp.condition) return comp;
        try {
          const { resources: compDocs } = await container.items.query({
            query: `SELECT c.year_built, c.quality_grade, c.condition FROM c WHERE c.id = @acct`,
            parameters: [{ name: "@acct", value: comp.acct }],
          }).fetchAll();
          if (compDocs.length > 0) {
            if (compDocs[0].year_built && !comp.yearBuilt) comp.yearBuilt = compDocs[0].year_built;
            if (compDocs[0].quality_grade) comp.qualityGrade = compDocs[0].quality_grade;
            if (compDocs[0].condition) comp.condition = compDocs[0].condition;
          }
        } catch { /* skip */ }
        return comp;
      })
    ...
    return {
      ...
      qualityGrade: prop.quality_grade || "",
    ...
    console.error("[getTarrantPropertyData] Error:", error);
    return null;
  }
}

// ──────────────────────────────────────────────────────────────────
// PDF HTML Generation — Tarrant County / TAD
// ──────────────────────────────────────────────────────────────────

function generateTarrantPdfHtml(data: TarrantPropertyData): string {
  ...
  const compsHtml = data.comps.map((c, i) => `
    <tr>
      ...
      <td class="right">${c.qualityGrade || "—"}</td>
      <td class="right">${c.condition || "—"}</td>
  ...
  const brief = generateTarrantBrief(data);
  ...
  <div class="page">
    ...
    <div class="header">
      ...
      <div class="header-right">
        ...
        <div>Tarrant County, Texas</div>
        <div>Protest Type: Uniform & Equal / Market Value</div>
        <div>Filing Body: TAD / Appraisal Review Board</div>
      </div>
    </div>
    ...
    <!-- Property Details — Tarrant has quality grade + condition instead of beds/baths -->
    <div class="breakdown-row">
      <div class="breakdown-card">
        ...
        <table class="breakdown-table">
          ...
          ${data.qualityGrade ? `<tr><td>Quality Grade</td><td>${data.qualityGrade}</td></tr>` : ""}
          ${data.condition ? `<tr><td>Condition</td><td>${data.condition}</td></tr>` : ""}
          ${data.stories ? `<tr><td>Stories</td><td>${escapeHtml(data.stories)}</td></tr>` : ""}
          ${data.pool ? `<tr><td>Pool</td><td>Yes</td></tr>` : ""}
          ${data.bldgClass ? `<tr><td>Property Class</td><td>${escapeHtml(data.bldgClass)}</td></tr>` : ""}
      ...
      <table class="comps-table">
        ...
        <tbody>
          <tr class="subject-row">
            ...
            <td class="right">${data.qualityGrade || "—"}</td>
            <td class="right">${data.condition || "—"}</td>
    ...
    <div class="section page-break">
      <div class="section-title">📋 How to Protest — Tarrant County Step by Step</div>
      <p style="font-size: 10px; color: #555; margin-bottom: 14px; line-height: 1.6;">Texas law gives every property owner the right to protest their appraised value. Tarrant Appraisal District (TAD) makes this easy — you can file online at tad.org. <strong>No attorney or agent needed.</strong></p>
      
      <div class="info-box warning">
        <p style="margin: 0;"><strong>⏰ Deadline:</strong> TAD protest season typically opens when appraisal notices are mailed (usually late March/early April). The deadline is typically May 15 or 30 days after your notice date, whichever is later. <strong>File as soon as you receive your notice for the best results.</strong> Verify your exact deadline at <a href="https://www.tad.org" class="step-link">tad.org</a>.</p>
      </div>
      ...
      <div class="steps">
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">TAD mails appraisal notices in <strong>late March to early April</strong>. Your notice will show your property's appraised value for the current year. Your account number is <strong style="font-family: monospace; background: #f0f0f0; padding: 1px 4px; border-radius: 3px;">${escapeHtml(data.acct)}</strong>.</div>
          </div>
        </div>
        <div class="step">
          <div class="step-number">2</div>
          <div class="step-content">
            <div class="step-title">File Online via TAD.org</div>
            <div class="step-desc">
              Go to <a href="https://www.tad.org/login" class="step-link">tad.org</a> → Log in or create an account → File your protest online<br/>
              ...
              • Submit your protest<br/>
              <br/>
              <strong>Alternative:</strong> You can also mail a Notice of Protest (Form 50-132) to:<br/>
              Tarrant Appraisal District, 2500 Handley-Ederville Rd, Fort Worth, TX 76118
            </div>
          </div>
        </div>
        <div class="step">
          ...
          <div class="step-content">
            <div class="step-title">Check for a Settlement Offer (Value Negotiation)</div>
            <div class="step-desc">
              After filing, TAD may offer a <strong>settlement</strong> through their online value negotiation tool.<br/>
        ...
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">
              If settlement doesn't work, you'll be scheduled for a <strong>Tarrant Appraisal Review Board (TARB) hearing</strong>. Hearings run from <strong>May through July</strong>. You can attend <strong>in person or by phone</strong>.<br/>
              ...
              • Highlight that comparable properties with <strong>similar quality grade and condition</strong> are appraised lower<br/>
        ...
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">
              The TARB panel will issue a decision. If you're not satisfied, you can appeal further to <strong>District Court</strong> or pursue <strong>binding arbitration</strong> (for properties under $5 million). However, most protests are resolved at the settlement or ARB stage.
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="section">
      ...
      <p style="font-size: 10px; color: #555; margin-bottom: 10px;">If you go to a TARB hearing, here's what to say. Keep it simple and factual:</p>
      <div style="background: #f8f9fa; border: 2px dashed #cbd5e1; border-radius: 8px; padding: 16px 18px; font-size: 10.5px; line-height: 1.7; color: #333; font-family: 'Georgia', serif;">
        ...
        <p style="margin-bottom: 8px;">"My property is rated <strong>${data.qualityGrade || "N/A"} quality</strong> in <strong>${data.condition || "N/A"} condition</strong>. I've identified ${data.comps.length} comparable properties in the same neighborhood (${escapeHtml(data.neighborhoodCode)}) with similar or better quality and condition ratings. The median appraised value of these comparable properties is <strong>$${data.compMedianPerSqft.toFixed(2)} per square foot</strong> — my property is appraised <strong>${data.overAssessedPct}% higher</strong> than comparable properties."</p>
        ...
        <p style="margin-bottom: 0;">"I have provided ${data.comps.length} comparable properties with their addresses, account numbers, quality grades, conditions, and appraised values as supporting evidence."</p>
      </div>
    </div>
    ...
    <div class="section">
      ...
      <div style="font-size: 10px; line-height: 1.7; color: #555; margin-top: 12px;">
        ...
        <p style="margin-bottom: 8px;"><strong>No Cost to File:</strong> Filing a protest with TAD is completely free. There is no penalty for protesting — even if your value isn't reduced.</p>
        <p style="margin-bottom: 8px;"><strong>Homestead Exemption:</strong> If you haven't already, apply for a <strong>homestead exemption</strong> through TAD. This is separate from your protest and can reduce your taxable value by $100,000 (effective 2024). Go to <a href="https://www.tad.org" class="step-link">tad.org</a> → "Forms" → "Homestead Exemption" to apply.</p>
...
function generateTarrantBrief(data: TarrantPropertyData): string {
  // Tarrant-specific: highlight quality grade + condition instead of beds/baths
  const features: string[] = [];
  if (data.qualityGrade) features.push(`${data.qualityGrade} quality grade`);
  if (data.condition) features.push(`${data.condition} condition`);
  if (data.stories) features.push(`${escapeHtml(data.stories)} story`);
  ...
  return `
    <p>The subject property at <strong>${escapeHtml(data.address)}</strong> (TAD Account: ${escapeHtml(data.acct)}), located in ${escapeHtml(data.city)}, TX, appraisal neighborhood ${escapeHtml(data.neighborhoodCode)}, is currently appraised at <strong>$${data.currentAssessment.toLocaleString()}</strong>, which equates to <strong>$${data.perSqft.toFixed(2)} per square foot</strong> of building area.${featuresStr}</p>
    
    <p>An analysis of ${data.comps.length} comparable properties within the same appraisal neighborhood — properties with similar square footage, quality grade, and condition — demonstrates that the subject property is appraised at a rate <strong>${data.overAssessedPct}% above the comparable median</strong> of $${data.compMedianPerSqft.toFixed(2)} per square foot. This constitutes an unequal appraisal Under <strong>Texas Tax Code §41.41(a)(2)</strong>, a property owner has the right to protest the appraised value on grounds that the property is unequally appraised. Under <strong>§42.26(a)</strong>, the appraised value of property must not exceed the median appraised value of a reasonable number of comparable properties appropriately adjusted.</p>
...
function buildQuickStartData(data: TarrantPropertyData): QuickStartData {
  return {
...
function buildCoverLetterData(data: TarrantPropertyData): CoverLetterData {
  return {
    ...
    state: "TX",
    filingBody: "TAD",
    filingBodyFull: "Tarrant Appraisal District",
    currentAssessment: data.currentAssessment,
    fairAssessment: data.fairAssessment,
    overAssessedPct: data.overAssessedPct,
    compCount: data.comps.length,
...
async function sendTarrantEmail(
  email: string,
  acct: string,
  quickStartPdf: Buffer,
  evidencePdf: Buffer,
  coverLetterPdf: Buffer,
  data: TarrantPropertyData,
  accessToken: string
): Promise<void> {
  ...
  await getResend().emails.send({
    ...
    html: `
      <div style="background: #f7f6f3; padding: 32px 16px;">
        ...
        <p style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #666; margin-bottom: 24px; font-size: 15px;">Everything you need to protest your property tax for <strong>${escapeHtml(data.address)}</strong> with the Tarrant Appraisal District.</p>
        ...
        <p style="font-size: 14px; margin-bottom: 8px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;"><strong>Three PDFs attached — here's how to use them:</strong></p>
            ...
            <div style="background: #f0fdf4; border: 1px solid #1a6b5a; border-radius: 8px; padding: 14px 16px; margin-bottom: 16px;">
              ...
              <p style="margin: 0; font-size: 14px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;"><strong>📎 Evidence Packet</strong> — Upload this to TAD when you file. It's your proof.</p>
            </div>
        <ul style="font-size: 14px; color: #555; margin-bottom: 24px; padding-left: 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
          ...
          <li>Step-by-step TAD online protest instructions</li>
          <li>Hearing script — exactly what to say</li>
          <li>Settlement guidance — how to evaluate TAD's offer</li>
        </ul>
...
// ──────────────────────────────────────────────────────────────────
// Token handling — tarrant:acct:sessionId format
// ──────────────────────────────────────────────────────────────────

function generateAccessToken(sessionId: string, acct: string): string {
  return _genToken(`tarrant:${acct}:${sessionId}`);
}
...
export async function GET(request: NextRequest) {
  ...
  if (accessToken) {
    ...
    const propertyData = await getTarrantPropertyData(tokenData.acct);
    if (!propertyData) {
      return NextResponse.json({ error: "Property not found" }, { status: 404 });
    }
    return NextResponse.json({ success: true, property: propertyData, token: accessToken, jurisdiction: "tarrant" });
  }
  ...
  if (sessionId) {
    try {
      ...
      if (!clientRef || !clientRef.startsWith("tarrant:")) {
        return NextResponse.json({ error: "Not a Tarrant property" }, { status: 400 });
      }
      const acct = clientRef.replace("tarrant:", "");
      const propertyData = await getTarrantPropertyData(acct);
      if (!propertyData) {
        return NextResponse.json({ error: "Property not found" }, { status: 404 });
      }
      const token = generateAccessToken(sessionId, acct);
      // Send email
      console.log(`[Tarrant] Email: ${email || "NONE"}, Acct: ${acct}, Session: ${sessionId}`);
      if (email) {
        try {
          console.log("[Tarrant] Generating PDFs...");
          ...
          const [quickStartPdf, evidencePdf, coverLetterPdf] = await Promise.all([
            ...
            generatePdf(generateTarrantEvidenceHtml(propertyData)),
            generatePdf(generateCoverLetterHtml(buildCoverLetterData(propertyData))),
          ]);
          console.log(`[Tarrant] PDFs generated: quick-start ${quickStartPdf.length} bytes, evidence ${evidencePdf.length} bytes`);
          await sendTarrantEmail(email, acct, quickStartPdf, evidencePdf, coverLetterPdf, propertyData, token);
          console.log("[Tarrant] Email sent successfully");
        } catch (emailErr) {
          console.error("[Tarrant] Email send error:", emailErr);
          // Don't fail the response — email is best-effort
        }
      } else {
        console.warn("[Tarrant] No email from Stripe session — skipping email send");
      }
      return NextResponse.json({ success: true, property: propertyData, token, email: email || null, jurisdiction: "tarrant" });
    } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  const html = generateTarrantEvidenceHtml(propertyData);
