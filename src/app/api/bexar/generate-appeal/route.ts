...
async function getBexarPropertyData(acct: string): Promise<BexarPropertyData | null> {
  try {
    const client = getCosmosClient();
    if (!client) return null;

    const container = client.database("overtaxed").container("bexar-properties");
    ...
    // Map comps
    const comps: BexarComp[] = (prop.comps || []).map((c: any) => ({
      acct: c.acct || "",
      address: c.address || "",
      sqft: c.sqft || 0,
      ...
      comps.map(async (comp) => {
        ...
        try {
          const { resources: compDocs } = await container.items.query({
            query: `SELECT c.year_built FROM c WHERE c.id = @acct`,
            parameters: [{ name: "@acct", value: comp.acct }],
          }).fetchAll();
          if (compDocs.length > 0 && compDocs[0].year_built) {
            comp.yearBuilt = compDocs[0].year_built;
          }
        } catch { /* skip */ }
        return comp;
      })
    ...
    console.error("[getBexarPropertyData] Error:", error);
    return null;
  }
}

// ──────────────────────────────────────────────────────────────────
// PDF HTML Generation — Bexar County
// ──────────────────────────────────────────────────────────────────

function generateBexarPdfHtml(data: BexarPropertyData): string {
  ...
  const brief = generateBexarBrief(data);
  ...
  <div class="page">
    ...
    <div class="header">
      ...
      <div class="header-right">
        ...
        <div>Filing Body: BCAD / Appraisal Review Board</div>
      </div>
    </div>
    ...
    <div class="breakdown-row">
      ...
      <div class="breakdown-card">
        ...
        <h4 style="margin-top: 12px;">Tax Impact (Est. 2.1% Rate)</h4>
        <table class="breakdown-table">
          <tr><td>Current Annual Tax</td><td>$${Math.round(data.currentAssessment * 0.021).toLocaleString()}</td></tr>
          <tr><td>After Reduction</td><td>$${Math.round(data.fairAssessment * 0.021).toLocaleString()}</td></tr>
    ...
    <div class="section page-break">
      <div class="section-title">📋 How to Protest — Bexar County Step by Step</div>
      <p style="font-size: 10px; color: #555; margin-bottom: 14px; line-height: 1.6;">Texas law gives every property owner the right to protest their appraised value. Bexar County Appraisal District (BCAD) offers online filing through bcadonline.org. <strong>No attorney or agent needed.</strong></p>
      
      <div class="info-box warning">
        <p style="margin: 0;"><strong>⏰ Deadline:</strong> Bexar County protest season typically opens when appraisal notices are mailed in April. The deadline is usually <strong>May 15</strong> (or 30 days after your notice date, whichever is later). <strong>File as soon as you receive your notice for the best results.</strong></p>
      </div>
      ...
      <div class="steps">
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">BCAD mails appraisal notices in <strong>April</strong>. Your notice will include a <strong>PIN number</strong> needed for online filing. Your property account: <strong style="font-family: monospace; background: #f0f0f0; padding: 1px 4px; border-radius: 3px;">${escapeHtml(data.acct)}</strong>.</div>
          </div>
        </div>
        <div class="step">
          <div class="step-number">2</div>
          <div class="step-content">
            <div class="step-title">File Online via BCAD Online</div>
            <div class="step-desc">
              Go to <span class="step-link">bcadonline.org</span> and log in with your account number and PIN.<br/>
        ...
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">
              After filing, BCAD will schedule an <strong>informal meeting</strong> where an appraiser will review your evidence and may offer a settlement.<br/>
    ...
    <div class="section">
      ...
      <div style="font-size: 10px; line-height: 1.7; color: #555; margin-top: 12px;">
        ...
        <p style="margin-bottom: 8px;"><strong>No Cost to File:</strong> Filing a protest with BCAD is completely free. There is no penalty for protesting — even if your value isn't reduced.</p>
        <p style="margin-bottom: 8px;"><strong>Homestead Exemption:</strong> If you haven't already, apply for a <strong>homestead exemption</strong> through BCAD. This is separate from your protest and can reduce your taxable value by $100,000 (effective 2024). Go to <span class="step-link">bcadonline.org</span> to apply.</p>
        <p style="margin-bottom: 0;"><strong>10% Homestead Cap:</strong> If you have a homestead exemption, your appraised value can't increase by more than 10% per year, regardless of what BCAD says the market value is. Protesting still matters because it lowers the <em>base</em> that future increases are calculated from.</p>
      </div>
    </div>
    
    <!-- BCAD Contact Info -->
    <div class="section">
      <div class="section-title">📞 Bexar County Appraisal District</div>
      <div style="font-size: 10px; line-height: 1.7; color: #555;">
        <p style="margin-bottom: 4px;"><strong>Address:</strong> 411 N Frio St, San Antonio, TX 78207</p>
        <p style="margin-bottom: 4px;"><strong>Website:</strong> <span class="step-link">bcad.org</span></p>
        <p style="margin-bottom: 4px;"><strong>Online Protests:</strong> <span class="step-link">bcadonline.org</span></p>
        <p style="margin-bottom: 4px;"><strong>Phone:</strong> (210) 224-2432</p>
      </div>
    </div>
    
    <!-- Footer -->
    <div class="footer">
      <p><strong>Disclaimer:</strong> This document provides comparable property data and analysis to support a property tax protest based on unequal appraisal under Texas Property Tax Code §42.26. It does not constitute legal advice. All appraisal data is sourced from Bexar County Appraisal District (BCAD) public records. Filing deadlines and procedures are subject to change — verify current deadlines at bcadonline.org before filing.</p>
...
function generateBexarBrief(data: BexarPropertyData): string {
  return `
    <p>The subject property at <strong>${escapeHtml(data.address)}</strong> (BCAD Account: ${escapeHtml(data.acct)}), located in appraisal neighborhood ${escapeHtml(data.neighborhoodCode)}, is currently appraised at <strong>$${data.currentAssessment.toLocaleString()}</strong>, which equates to <strong>$${data.perSqft.toFixed(2)} per square foot</strong> of building area.</p>
    
    <p>An analysis of ${data.comps.length} comparable properties within the same appraisal neighborhood demonstrates that the subject property is appraised at a rate <strong>${data.overAssessedPct}% above the comparable median</strong> of $${data.compMedianPerSqft.toFixed(2)} per square foot. This constitutes an unequal appraisal. Under <strong>Texas Property Tax Code §42.26</strong>, the appraised value of property must not exceed the median appraised value of a reasonable number of comparable properties appropriately adjusted.</p>
...
function buildQuickStartData(data: BexarPropertyData): QuickStartData {
  return {
...
function buildCoverLetterData(data: BexarPropertyData): CoverLetterData {
  return {
    ...
    state: "TX",
    filingBody: "BCAD",
    filingBodyFull: "Bexar Appraisal District",
    currentAssessment: data.currentAssessment,
    fairAssessment: data.fairAssessment,
    overAssessedPct: data.overAssessedPct,
    compCount: data.comps.length,
...
async function sendBexarEmail(
  email: string,
  acct: string,
  quickStartPdf: Buffer,
  evidencePdf: Buffer,
  coverLetterPdf: Buffer,
  data: BexarPropertyData,
  accessToken: string
): Promise<void> {
  ...
  await getResend().emails.send({
    ...
    html: `
      <div style="background: #f7f6f3; padding: 32px 16px;">
        ...
        <p style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #666; margin-bottom: 24px; font-size: 15px;">Everything you need to protest your property tax for <strong>${escapeHtml(data.address)}</strong> with the Bexar County Appraisal District.</p>
        ...
        <div style="background: #fef3c7; border: 1px solid #f59e0b; border-radius: 8px; padding: 14px 16px; margin-bottom: 24px;">
          <p style="margin: 0; font-size: 13px; color: #92400e; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;"><strong>⏰ Deadline:</strong> File your protest by May 15 (or 30 days after your notice, whichever is later). Don't wait!</p>
        </div>
        ...
        <p style="font-size: 14px; margin-bottom: 8px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;"><strong>Three PDFs attached — here's how to use them:</strong></p>
            ...
            <div style="background: #f0fdf4; border: 1px solid #1a6b5a; border-radius: 8px; padding: 14px 16px; margin-bottom: 16px;">
              ...
              <p style="margin: 0; font-size: 14px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;"><strong>📎 Evidence Packet</strong> — Upload this to BCAD when you file. It's your proof.</p>
            </div>
        <ul style="font-size: 14px; color: #555; margin-bottom: 24px; padding-left: 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
          ...
          <li>Written "Uniform & Equal" argument citing Texas Property Tax Code §42.26</li>
          <li>Step-by-step BCAD online filing instructions</li>
          <li>Hearing script — exactly what to say</li>
          <li>BCAD contact information and portal links</li>
        </ul>
...
function generateAccessToken(sessionId: string, acct: string): string {
  return _genToken(`bexar:${acct}:${sessionId}`);
}
...
function verifyAccessToken(token: string): { acct: string; sessionId: string } | null {
  ...
  if (parts.length < 3 || parts[0] !== "bexar") return null;
...
export async function GET(request: NextRequest) {
  ...
  if (accessToken) {
    ...
    const propertyData = await getBexarPropertyData(tokenData.acct);
    if (!propertyData) {
      return NextResponse.json({ error: "Property not found" }, { status: 404 });
    }
    return NextResponse.json({ success: true, property: propertyData, token: accessToken, jurisdiction: "bexar" });
  }
  ...
  if (sessionId) {
    try {
      ...
      if (!clientRef || !clientRef.startsWith("bexar:")) {
        return NextResponse.json({ error: "Not a Bexar County property" }, { status: 400 });
      }
      const acct = clientRef.replace("bexar:", "");
      const propertyData = await getBexarPropertyData(acct);
      if (!propertyData) {
        return NextResponse.json({ error: "Property not found" }, { status: 404 });
      }
      ...
      if (email) {
        try {
          ...
          const [quickStartPdf, evidencePdf, coverLetterPdf] = await Promise.all([
            ...
            generatePdf(generateBexarEvidenceHtml(propertyData)),
            generatePdf(generateCoverLetterHtml(buildCoverLetterData(propertyData))),
          ]);
          await sendBexarEmail(email, acct, quickStartPdf, evidencePdf, coverLetterPdf, propertyData, token);
        } catch (emailErr) {
          console.error("Email send error:", emailErr);
        }
      }
      return NextResponse.json({ success: true, property: propertyData, token, email: email || null, jurisdiction: "bexar" });
    } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  const html = generateBexarEvidenceHtml(propertyData);
