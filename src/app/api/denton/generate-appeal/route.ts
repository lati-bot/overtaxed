...
import { DentonEvidenceData, generateDentonEvidenceHtml } from "@/lib/evidence-packet-denton";
...
// ──────────────────────────────────────────────────────────────────
// Denton Property Data Types
// ──────────────────────────────────────────────────────────────────

interface DentonComp {
  acct: string;
  address: string;
  sqft: number;
  assessedVal: number;
...
async function getDentonPropertyData(acct: string): Promise<DentonPropertyData | null> {
  try {
    ...
    // Map comps — Denton has no beds/baths
    const comps: DentonComp[] = (prop.comps || []).map((c: any) => ({
      acct: c.acct || "",
      address: c.address || "",
      sqft: c.sqft || 0,
    ...
    console.error("[getDentonPropertyData] Error:", error);
    return null;
  }
}

// ──────────────────────────────────────────────────────────────────
// PDF HTML Generation — Denton County / DCAD
// ──────────────────────────────────────────────────────────────────

function generateDentonPdfHtml(data: DentonPropertyData): string {
  ...
  const brief = generateDentonBrief(data);
  ...
  <div class="page">
    ...
    <div class="header">
      ...
      <div class="header-right">
        ...
        <div>Denton County, Texas</div>
        <div>Protest Type: Uniform & Equal / Market Value</div>
        <div>Filing Body: Denton CAD / Appraisal Review Board</div>
      </div>
    </div>
    ...
    <!-- Property Details — Denton has no beds/baths -->
    <div class="breakdown-row">
      <div class="breakdown-card">
        ...
        <table class="breakdown-table">
          ...
          <tr><td>Neighborhood / Subdivision</td><td>${escapeHtml(data.neighborhoodCode)}</td></tr>
        </table>
      </div>
      <div class="breakdown-card">
    ...
    <div class="section page-break">
      <div class="section-title">📋 How to Protest — Denton County Step by Step</div>
      <p style="font-size: 10px; color: #555; margin-bottom: 14px; line-height: 1.6;">Texas law gives every property owner the right to protest their appraised value. Denton Central Appraisal District (DCAD) makes this easy — you can file online through their E-File system. <strong>No attorney or agent needed.</strong></p>
      
      <div class="info-box warning">
        <p style="margin: 0;"><strong>⏰ Deadline:</strong> DCAD protest season typically opens when appraisal notices are mailed (usually mid-April). The deadline is typically May 15 or 30 days after your notice date, whichever is later. <strong>File as soon as you receive your notice for the best results.</strong> Verify your exact deadline at <a href="https://www.dentoncad.com" class="step-link">dentoncad.com</a>.</p>
      </div>
      ...
      <div class="steps">
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">DCAD mails appraisal notices in <strong>mid-April</strong>. Your notice will show your property's appraised value for the current year. Your account number is <strong style="font-family: monospace; background: #f0f0f0; padding: 1px 4px; border-radius: 3px;">${escapeHtml(data.acct)}</strong>.</div>
          </div>
        </div>
        <div class="step">
          <div class="step-number">2</div>
          <div class="step-content">
            <div class="step-title">File Online via DCAD E-File</div>
            <div class="step-desc">
              Go to <a href="https://appeals.dentoncad.com" class="step-link">appeals.dentoncad.com</a> → Create an account or log in → File your protest online<br/>
              • Use your <strong>Property ID</strong>: <strong style="font-family: monospace; background: #f0f0f0; padding: 1px 4px; border-radius: 3px;">${escapeHtml(data.acct)}</strong><br/>
              ...
              Denton Central Appraisal District, PO Box 2816, Denton, TX 76202
            </div>
          </div>
        </div>
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">
              After filing, DCAD may offer a <strong>settlement</strong> through their E-File system — filing early increases your chances of receiving an online settlement offer.<br/>
      ...
      <div class="info-box info">
        <p style="margin: 0;"><strong>📬 File by Mail (Alternative):</strong> You can also mail your protest to: <strong>Denton Central Appraisal District, PO Box 2816, Denton, TX 76202</strong>. Include a completed Notice of Protest form (available at dentoncad.com) along with this evidence package. You may also visit the office in person at <strong>3911 Morse St, Denton, TX 76208</strong>. Online filing via E-File is faster and recommended.</p>
      </div>
    </div>
    ...
    <div class="section">
      ...
      <div style="font-size: 10px; line-height: 1.7; color: #555; margin-top: 12px;">
        ...
        <p style="margin-bottom: 8px;"><strong>No Cost to File:</strong> Filing a protest with DCAD is completely free. There is no penalty for protesting — even if your value isn't reduced.</p>
        <p style="margin-bottom: 8px;"><strong>Homestead Exemption:</strong> If you haven't already, apply for a <strong>homestead exemption</strong> through DCAD. This is separate from your protest and can reduce your taxable value by $100,000 (effective 2024). Go to <a href="https://www.dentoncad.com" class="step-link">dentoncad.com</a> → "Forms" to apply.</p>
...
function generateDentonBrief(data: DentonPropertyData): string {
  return `
    <p>The subject property at <strong>${escapeHtml(data.address)}</strong> (DCAD Account: ${escapeHtml(data.acct)}), located in ${escapeHtml(data.city)}, TX, appraisal neighborhood ${escapeHtml(data.neighborhoodCode)}, is currently appraised at <strong>$${data.currentAssessment.toLocaleString()}</strong>, which equates to <strong>$${data.perSqft.toFixed(2)} per square foot</strong> of building area. The property is a ${data.sqft.toLocaleString()} sq ft residence${data.yearBuilt ? ` built in ${data.yearBuilt}` : ""}.</p>
...
function buildQuickStartData(data: DentonPropertyData): QuickStartData {
  return {
    ...
    portalScreenshotDataUri: getPortalScreenshotDataUri("Denton") ?? undefined,
  };
}

function buildCoverLetterData(data: DentonPropertyData): CoverLetterData {
  return {
    ...
    state: "TX",
    filingBody: "DCAD",
    filingBodyFull: "Denton Central Appraisal District",
    currentAssessment: data.currentAssessment,
    fairAssessment: data.fairAssessment,
    overAssessedPct: data.overAssessedPct,
    compCount: data.comps.length,
...
async function sendDentonEmail(
  email: string,
  acct: string,
  quickStartPdf: Buffer,
  evidencePdf: Buffer,
  coverLetterPdf: Buffer,
  data: DentonPropertyData,
  accessToken: string
): Promise<void> {
  ...
  await getResend().emails.send({
    ...
    html: `
      <div style="background: #f7f6f3; padding: 32px 16px;">
        ...
        <p style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #666; margin-bottom: 24px; font-size: 15px;">Everything you need to protest your property tax for <strong>${escapeHtml(data.address)}</strong> with the Denton Central Appraisal District.</p>
        ...
        <p style="font-size: 14px; margin-bottom: 8px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;"><strong>Three PDFs attached — here's how to use them:</strong></p>
            ...
            <div style="background: #f0fdf4; border: 1px solid #1a6b5a; border-radius: 8px; padding: 14px 16px; margin-bottom: 16px;">
              ...
              <p style="margin: 0; font-size: 14px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;"><strong>📎 Evidence Packet</strong> — Upload this to DCAD when you file. It's your proof.</p>
            </div>
        <ul style="font-size: 14px; color: #555; margin-bottom: 24px; padding-left: 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
          ...
          <li>Step-by-step DCAD E-File protest instructions</li>
...
// ──────────────────────────────────────────────────────────────────
// Token handling — denton:acct:sessionId format
// ──────────────────────────────────────────────────────────────────

function generateAccessToken(sessionId: string, acct: string): string {
  return _genToken(`denton:${acct}:${sessionId}`);
}
...
function verifyAccessToken(token: string): { acct: string; sessionId: string } | null {
  ...
  if (parts.length < 3 || parts[0] !== "denton") return null;
...
export async function GET(request: NextRequest) {
  ...
  if (accessToken) {
    ...
    const propertyData = await getDentonPropertyData(tokenData.acct);
    if (!propertyData) {
      return NextResponse.json({ error: "Property not found" }, { status: 404 });
    }
    return NextResponse.json({ success: true, property: propertyData, token: accessToken, jurisdiction: "denton" });
  }
  ...
  if (sessionId) {
    try {
      ...
      if (!clientRef || !clientRef.startsWith("denton:")) {
        return NextResponse.json({ error: "Not a Denton County property" }, { status: 400 });
      }
      const acct = clientRef.replace("denton:", "");
      const propertyData = await getDentonPropertyData(acct);
      if (!propertyData) {
        return NextResponse.json({ error: "Property not found" }, { status: 404 });
      }
      const token = generateAccessToken(sessionId, acct);
      // Send email
      console.log(`[Denton] Email: ${email || "NONE"}, Acct: ${acct}, Session: ${sessionId}`);
      if (email) {
        try {
          console.log("[Denton] Generating PDFs...");
          ...
          const [quickStartPdf, evidencePdf, coverLetterPdf] = await Promise.all([
            ...
            generatePdf(generateDentonEvidenceHtml(propertyData)),
            generatePdf(generateCoverLetterHtml(buildCoverLetterData(propertyData))),
          ]);
          console.log(`[Denton] PDFs generated: quick-start ${quickStartPdf.length} bytes, evidence ${evidencePdf.length} bytes`);
          await sendDentonEmail(email, acct, quickStartPdf, evidencePdf, coverLetterPdf, propertyData, token);
          console.log("[Denton] Email sent successfully");
        } catch (emailErr) {
          console.error("[Denton] Email send error:", emailErr);
          // Don't fail the response — email is best-effort
        }
      } else {
        console.warn("[Denton] No email from Stripe session — skipping email send");
      }
      return NextResponse.json({ success: true, property: propertyData, token, email: email || null, jurisdiction: "denton" });
    } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  const html = generateDentonEvidenceHtml(propertyData);
