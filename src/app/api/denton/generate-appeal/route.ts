    ...
    <div class="breakdown-row">
      <div class="breakdown-card">
        ...
        <table class="breakdown-table">
          ...
          <tr><td>Neighborhood / Subdivision</td><td>${data.neighborhoodCode}</td></tr>
        </table>
      </div>
      <div class="breakdown-card">
      ...
      <div class="steps">
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">DCAD mails appraisal notices in <strong>mid-April</strong>. Your notice will show your property's appraised value for the current year. Your account number is <strong style="font-family: monospace; background: #f0f0f0; padding: 1px 4px; border-radius: 3px;">${data.acct}</strong>.</div>
          </div>
        </div>
        <div class="step">
...
function generateDentonBrief(data: DentonPropertyData): string {
  return `
    <p>The subject property at <strong>${data.address}</strong> (DCAD Account: ${data.acct}), located in ${data.city}, TX, appraisal neighborhood ${data.neighborhoodCode}, is currently appraised at <strong>$${data.currentAssessment.toLocaleString()}</strong>, which equates to <strong>$${data.perSqft.toFixed(2)} per square foot</strong> of building area. The property is a ${data.sqft.toLocaleString()} sq ft residence${data.yearBuilt ? ` built in ${data.yearBuilt}` : ""}.</p>
  ...
  await getResend().emails.send({
    ...
    html: `
      <div style="background: #f7f6f3; padding: 32px 16px;">
        ...
        <p style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #666; margin-bottom: 24px; font-size: 15px;">Everything you need to protest your property tax for <strong>${data.address}</strong> with the Denton Central Appraisal District.</p>
...
function generateAccessToken(sessionId: string, acct: string): string {
  const data = `${sessionId}:denton:${acct}:${Date.now()}`;
  const hash = crypto.createHash("sha256").update(data + process.env.STRIPE_SECRET_KEY).digest("hex");
  return `${Buffer.from(`denton:${acct}:${sessionId}`).toString("base64url")}.${hash.slice(0, 16)}`;
}
