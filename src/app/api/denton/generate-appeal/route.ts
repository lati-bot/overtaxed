...
export async function GET(request: NextRequest) {
  ...
  if (sessionId) {
    try {
      ...
      if (email) {
        try {
          console.log("[Denton] Generating PDF...");
          const pdfBuffer = await generatePdf(generateDentonPdfHtml(propertyData));
          console.log(`[Denton] PDF generated: ${pdfBuffer.length} bytes`);
          await sendDentonEmail(email, acct, pdfBuffer, propertyData, token);
...
export async function POST(request: NextRequest) {
  ...
  const html = generateDentonPdfHtml(propertyData);
