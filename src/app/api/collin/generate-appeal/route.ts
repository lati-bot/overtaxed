...
async function sendCollinEmail(
  ...
  await getResend().emails.send({
    ...
    to: email,
    subject: `Property Tax Protest Package — ${data.address} — Collin County`,
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; color: #1a1a2e;">
        ...
        <p style="color: #666; margin-bottom: 24px; font-size: 15px;">Everything you need to protest your property tax for <strong>${data.address}</strong> with the Collin Central Appraisal District.</p>
...
function generateAccessToken(sessionId: string, acct: string): string {
  const data = `${sessionId}:collin:${acct}:${Date.now()}`;
  const hash = crypto.createHash("sha256").update(data + process.env.STRIPE_SECRET_KEY).digest("hex");
  return `${Buffer.from(`collin:${acct}:${sessionId}`).toString("base64url")}.${hash.slice(0, 16)}`;
}
