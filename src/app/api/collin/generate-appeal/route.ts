...
import { CollinEvidenceData, generateCollinEvidenceHtml } from "@/lib/evidence-packet-collin";
      ...
      <div class="steps">
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">CCAD mails appraisal notices in <strong>mid-April</strong>. Your notice will show your property's appraised value for the current year. Your account number is <strong style="font-family: monospace; background: #f0f0f0; padding: 1px 4px; border-radius: 3px;">${escapeHtml(data.acct)}</strong>.</div>
          </div>
        </div>
        <div class="step">
...
function generateCollinBrief(data: CollinPropertyData): string {
  ...
  return `
    <p>The subject property at <strong>${escapeHtml(data.address)}</strong> (CCAD Account: ${escapeHtml(data.acct)}), located in ${escapeHtml(data.city)}, TX, appraisal neighborhood ${escapeHtml(data.neighborhoodCode)}, is currently appraised at <strong>$${data.currentAssessment.toLocaleString()}</strong>, which equates to <strong>$${data.perSqft.toFixed(2)} per square foot</strong> of building area.${featuresStr}</p>
...
function buildQuickStartData(data: CollinPropertyData): QuickStartData {
  return {
    ...
    portalScreenshotDataUri: getPortalScreenshotDataUri("Collin") ?? undefined,
  };
}

function buildCoverLetterData(data: CollinPropertyData): CoverLetterData {
  return {
    ...
    state: "TX",
    filingBody: "CCAD",
    filingBodyFull: "Collin Central Appraisal District",
    currentAssessment: data.currentAssessment,
    fairAssessment: data.fairAssessment,
    overAssessedPct: data.overAssessedPct,
    compCount: data.comps.length,
  ...
  await getResend().emails.send({
    ...
    html: `
      <div style="background: #f7f6f3; padding: 32px 16px;">
        ...
        <p style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #666; margin-bottom: 24px; font-size: 15px;">Everything you need to protest your property tax for <strong>${escapeHtml(data.address)}</strong> with the Collin Central Appraisal District.</p>
        ...
        <p style="font-size: 14px; margin-bottom: 8px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;"><strong>Three PDFs attached — here's how to use them:</strong></p>
            ...
            <div style="background: #f0fdf4; border: 1px solid #1a6b5a; border-radius: 8px; padding: 14px 16px; margin-bottom: 16px;">
              ...
              <p style="margin: 0; font-size: 14px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;"><strong>📎 Evidence Packet</strong> — Upload this to CCAD when you file. It's your proof.</p>
            </div>
...
function generateAccessToken(sessionId: string, acct: string): string {
  return _genToken(`collin:${acct}:${sessionId}`);
}
...
export async function GET(request: NextRequest) {
  ...
  if (sessionId) {
    try {
      ...
      if (email) {
        try {
          console.log("[Collin] Generating PDFs...");
          ...
          const [quickStartPdf, evidencePdf, coverLetterPdf] = await Promise.all([
            ...
            generatePdf(generateCollinEvidenceHtml(propertyData)),
            generatePdf(generateCoverLetterHtml(buildCoverLetterData(propertyData))),
          ]);
          console.log(`[Collin] PDFs generated: quick-start ${quickStartPdf.length} bytes, evidence ${evidencePdf.length} bytes`);
          await sendCollinEmail(email, acct, quickStartPdf, evidencePdf, coverLetterPdf, propertyData, token);
...
export async function POST(request: NextRequest) {
  ...
  const html = generateCollinEvidenceHtml(propertyData);
