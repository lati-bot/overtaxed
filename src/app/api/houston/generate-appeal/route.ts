...
// ──────────────────────────────────────────────────────────────────
// Houston Property Data Types
// ──────────────────────────────────────────────────────────────────

interface HoustonComp {
  acct: string;
  address: string;
  sqft: number;
  assessedVal: number;
...
async function getHoustonPropertyData(acct: string): Promise<HoustonPropertyData | null> {
  try {
    ...
    // Map comps
    const comps: HoustonComp[] = (prop.comps || []).map((c: any) => ({
      acct: c.acct || "",
      address: c.address || "",
      sqft: c.sqft || 0,
    ...
    // Now enrich comps with year_built from their own Cosmos documents
    ...
    console.error("[getHoustonPropertyData] Error:", error);
    return null;
  }
}

// ──────────────────────────────────────────────────────────────────
// PDF HTML Generation — Houston / Harris County
// ──────────────────────────────────────────────────────────────────

function generateHoustonPdfHtml(data: HoustonPropertyData): string {
  ...
  const brief = generateHoustonBrief(data);
...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .info-box.tip { background: #f0fdf4; border: 2px solid #1a6b5a; }
  ...
  <div class="page">
    ...
    <div class="header">
      ...
      <div class="header-right">
        ...
        <div>Filing Body: HCAD / Appraisal Review Board</div>
      </div>
    </div>
    ...
    <div class="section page-break">
      <div class="section-title">📋 How to Protest — Harris County Step by Step</div>
      ...
      <div class="steps">
        ...
        <div class="step">
          ...
          <div class="step-content">
            <div class="step-title">Check for an iSettle Offer</div>
            <div class="step-desc">
              After filing, HCAD may send you a <strong>settlement offer via iSettle</strong> (check your email and your iFile account). This is HCAD's automated settlement system — they may offer to reduce your value without a hearing.<br/>
        ...
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">
              If iSettle doesn't work, you'll be scheduled for an <strong>Appraisal Review Board (ARB) hearing</strong>. Hearings run from <strong>May through July</strong>. You can attend <strong>in person or by phone</strong>.<br/>
        ...
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">
              The ARB panel will issue a decision, usually within a few weeks. If you're not satisfied with the result, you can appeal further to <strong>District Court</strong> or pursue <strong>binding arbitration</strong> (for properties under $5 million). However, most protests are resolved at the iSettle or ARB stage.
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="section">
      ...
      <div style="font-size: 10px; line-height: 1.7; color: #555; margin-top: 12px;">
        ...
        <p style="margin-bottom: 8px;"><strong>No Cost to File:</strong> Filing a protest with HCAD is completely free. There is no penalty for protesting — even if your value isn't reduced.</p>
        <p style="margin-bottom: 8px;"><strong>Homestead Exemption:</strong> If you haven't already, apply for a <strong>homestead exemption</strong> through HCAD. This is separate from your protest and can reduce your taxable value by $100,000 (effective 2024). Go to <span class="step-link">hcad.org</span> → "Exemptions" to apply.</p>
        <p style="margin-bottom: 0;"><strong>10% Homestead Cap:</strong> If you have a homestead exemption, your appraised value can't increase by more than 10% per year, regardless of what HCAD says the market value is. Protesting still matters because it lowers the <em>base</em> that future increases are calculated from.</p>
      </div>
    </div>
        ...
        <div style="max-width: 600px; margin: 0 auto;">
          ...
          <div style="background: #ffffff; padding: 28px 24px; border: 1px solid #e2e2e0; border-top: none; border-radius: 0 0 12px 12px;">
            ...
            <ul style="font-size: 14px; color: #555; margin-bottom: 24px; padding-left: 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
              ...
              <li>Step-by-step iFile and ARB hearing instructions</li>
...
function generateAccessToken(sessionId: string, acct: string): string {
  const data = `${sessionId}:houston:${acct}:${Date.now()}`;
  const hash = crypto.createHash("sha256").update(data + process.env.STRIPE_SECRET_KEY).digest("hex");
  return `${Buffer.from(`houston:${acct}:${sessionId}`).toString("base64url")}.${hash.slice(0, 16)}`;
}
...
export async function GET(request: NextRequest) {
  ...
  if (accessToken) {
    ...
    const propertyData = await getHoustonPropertyData(tokenData.acct);
    if (!propertyData) {
      return NextResponse.json({ error: "Property not found" }, { status: 404 });
    }
    return NextResponse.json({ success: true, property: propertyData, token: accessToken, jurisdiction: "houston" });
  }
  ...
  if (sessionId) {
    try {
      ...
      if (!clientRef || !clientRef.startsWith("houston:")) {
        return NextResponse.json({ error: "Not a Houston property" }, { status: 400 });
      }
      const acct = clientRef.replace("houston:", "");
      const propertyData = await getHoustonPropertyData(acct);
      if (!propertyData) {
        return NextResponse.json({ error: "Property not found" }, { status: 404 });
      }
...
export async function POST(request: NextRequest) {
  ...
  const html = generateHoustonPdfHtml(propertyData);
