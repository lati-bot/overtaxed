...
export async function GET(request: NextRequest) {
  ...
  if (sessionId) {
    try {
      ...
      if (email) {
        try {
          const pdfBuffer = await generatePdf(generateFortBendPdfHtml(propertyData));
          await sendFortBendEmail(email, acct, pdfBuffer, propertyData, token);
        } catch (emailErr) {
...
export async function POST(request: NextRequest) {
  ...
  const html = generateFortBendPdfHtml(propertyData);
