...
import { FortBendEvidenceData, generateFortBendEvidenceHtml } from "@/lib/evidence-packet-fortbend";
...
// ──────────────────────────────────────────────────────────────────
// Fort Bend Property Data Types
// ──────────────────────────────────────────────────────────────────

interface FortBendComp {
  acct: string;
  address: string;
  sqft: number;
  assessedVal: number;
...
interface FortBendPropertyData {
  acct: string;
  address: string;
  city: string;
  state: string;
...
async function getFortBendPropertyData(acct: string): Promise<FortBendPropertyData | null> {
  try {
    ...
    // Map comps
    const comps: FortBendComp[] = (prop.comps || []).map((c: any) => ({
      acct: c.acct || "",
      address: c.address || "",
      sqft: c.sqft || 0,
    ...
    console.error("[getFortBendPropertyData] Error:", error);
    return null;
  }
}

// ──────────────────────────────────────────────────────────────────
// PDF HTML Generation — Fort Bend County
// ──────────────────────────────────────────────────────────────────

function generateFortBendPdfHtml(data: FortBendPropertyData): string {
  ...
  const brief = generateFortBendBrief(data);
  ...
  <div class="page">
    ...
    <div class="header">
      ...
      <div class="header-right">
        ...
        <div>Fort Bend County, Texas</div>
        <div>Protest Type: Uniform & Equal / Market Value</div>
        <div>Filing Body: FBCAD / Appraisal Review Board</div>
      </div>
    </div>
    ...
    <div class="title-block">
      ...
      <div class="property-meta">
        ...
        ${data.beds ? `<span><strong>Beds:</strong> ${data.beds}</span>` : ""}
        ${data.baths ? `<span><strong>Baths:</strong> ${data.baths}</span>` : ""}
      </div>
    </div>
    ...
    <div class="breakdown-row">
      <div class="breakdown-card">
        ...
        <table class="breakdown-table">
          ...
          ${data.beds ? `<tr><td>Bedrooms</td><td>${data.beds}</td></tr>` : ""}
          ${data.baths ? `<tr><td>Bathrooms</td><td>${data.baths}</td></tr>` : ""}
      ...
      <div class="breakdown-card">
        ...
        <h4 style="margin-top: 12px;">Tax Impact (Est. 2.48% Rate)</h4>
        <table class="breakdown-table">
          <tr><td>Current Annual Tax</td><td>$${Math.round(data.currentAssessment * 0.0248).toLocaleString()}</td></tr>
          <tr><td>After Reduction</td><td>$${Math.round(data.fairAssessment * 0.0248).toLocaleString()}</td></tr>
    ...
    <div class="section page-break">
      <div class="section-title">📋 How to Protest — Fort Bend County Step by Step</div>
      <p style="font-size: 10px; color: #555; margin-bottom: 14px; line-height: 1.6;">Texas law gives every property owner the right to protest their appraised value. Fort Bend CAD offers online filing and informal meetings. <strong>No attorney or agent needed.</strong></p>
      
      <div class="info-box warning">
        <p style="margin: 0;"><strong>⏰ Deadline:</strong> Fort Bend County protest season typically opens in late March when appraisal notices are mailed. The deadline is usually May 15 (or 30 days after your notice date, whichever is later). <strong>File as soon as you receive your notice for the best results.</strong></p>
      </div>
      ...
      <div class="steps">
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">FBCAD mails appraisal notices in <strong>late March to early April</strong>. Your notice will include a <strong>PIN number</strong> needed for online filing. Your property account: <strong style="font-family: monospace; background: #f0f0f0; padding: 1px 4px; border-radius: 3px;">${escapeHtml(data.acct)}</strong>.</div>
          </div>
        </div>
        <div class="step">
          <div class="step-number">2</div>
          <div class="step-content">
            <div class="step-title">File Online via FBCAD Portal</div>
            <div class="step-desc">
              Go to <span class="step-link">fbcad.org</span> and click <strong>"Property Owner's Login"</strong> → <strong>"Online Appeals"</strong><br/>
              • Enter your <strong>PIN number</strong> from your appraisal notice<br/>
        ...
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">
              FBCAD's online portal combines electronic filing with an <strong>online informal meeting</strong>. After filing, an appraiser will review your evidence and may offer a settlement.<br/>
    ...
    <div class="section">
      ...
      <div style="font-size: 10px; line-height: 1.7; color: #555; margin-top: 12px;">
        ...
        <p style="margin-bottom: 8px;"><strong>No Cost to File:</strong> Filing a protest with FBCAD is completely free. There is no penalty for protesting — even if your value isn't reduced.</p>
        <p style="margin-bottom: 8px;"><strong>Homestead Exemption:</strong> If you haven't already, apply for a <strong>homestead exemption</strong> through FBCAD. This is separate from your protest and can reduce your taxable value by $100,000 (effective 2024). Go to <span class="step-link">fbcad.org</span> → "Property Owner's Login" → "Exemptions/Rendition Portal" to apply.</p>
        <p style="margin-bottom: 0;"><strong>10% Homestead Cap:</strong> If you have a homestead exemption, your appraised value can't increase by more than 10% per year, regardless of what FBCAD says the market value is. Protesting still matters because it lowers the <em>base</em> that future increases are calculated from.</p>
      </div>
    </div>
    
    <!-- FBCAD Contact Info -->
    <div class="section">
      <div class="section-title">📞 Fort Bend Central Appraisal District</div>
      <div style="font-size: 10px; line-height: 1.7; color: #555;">
        <p style="margin-bottom: 4px;"><strong>Address:</strong> 2801 B.F. Terry Blvd, Rosenberg, TX 77471</p>
        <p style="margin-bottom: 4px;"><strong>Website:</strong> <span class="step-link">fbcad.org</span></p>
        <p style="margin-bottom: 4px;"><strong>Phone:</strong> (281) 344-8623</p>
        <p style="margin-bottom: 4px;"><strong>Online Appeals:</strong> <span class="step-link">fbcad.org/property-owners-login</span> → "Online Appeals"</p>
        <p style="margin-bottom: 4px;"><strong>Email:</strong> info@fbcad.org</p>
      </div>
    </div>
...
function generateFortBendBrief(data: FortBendPropertyData): string {
  return `
    <p>The subject property at <strong>${escapeHtml(data.address)}</strong> (FBCAD Account: ${escapeHtml(data.acct)}), located in appraisal neighborhood ${escapeHtml(data.neighborhoodCode)}, is currently appraised at <strong>$${data.currentAssessment.toLocaleString()}</strong>, which equates to <strong>$${data.perSqft.toFixed(2)} per square foot</strong> of building area.</p>
...
function buildQuickStartData(data: FortBendPropertyData): QuickStartData {
  return {
    ...
    portalScreenshotDataUri: getPortalScreenshotDataUri("Fort Bend") ?? undefined,
  };
}

function buildCoverLetterData(data: FortBendPropertyData): CoverLetterData {
  return {
    ...
    state: "TX",
    filingBody: "FBCAD",
    filingBodyFull: "Fort Bend Central Appraisal District",
    currentAssessment: data.currentAssessment,
    fairAssessment: data.fairAssessment,
    overAssessedPct: data.overAssessedPct,
    compCount: data.comps.length,
...
async function sendFortBendEmail(
  email: string,
  acct: string,
  quickStartPdf: Buffer,
  evidencePdf: Buffer,
  coverLetterPdf: Buffer,
  data: FortBendPropertyData,
  accessToken: string
): Promise<void> {
  ...
  await getResend().emails.send({
    ...
    html: `
      <div style="background: #f7f6f3; padding: 32px 16px;">
        ...
        <p style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #666; margin-bottom: 24px; font-size: 15px;">Everything you need to protest your property tax for <strong>${escapeHtml(data.address)}</strong> with the Fort Bend Central Appraisal District.</p>
        ...
        <p style="font-size: 14px; margin-bottom: 8px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;"><strong>Three PDFs attached — here's how to use them:</strong></p>
            ...
            <div style="background: #f0fdf4; border: 1px solid #1a6b5a; border-radius: 8px; padding: 14px 16px; margin-bottom: 16px;">
              ...
              <p style="margin: 0; font-size: 14px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;"><strong>📎 Evidence Packet</strong> — Upload this to FBCAD when you file. It's your proof.</p>
            </div>
...
function generateAccessToken(sessionId: string, acct: string): string {
  return _genToken(`fortbend:${acct}:${sessionId}`);
}
...
function verifyAccessToken(token: string): { acct: string; sessionId: string } | null {
  ...
  if (parts.length < 3 || parts[0] !== "fortbend") return null;
...
export async function GET(request: NextRequest) {
  ...
  if (accessToken) {
    ...
    const propertyData = await getFortBendPropertyData(tokenData.acct);
    if (!propertyData) {
      return NextResponse.json({ error: "Property not found" }, { status: 404 });
    }
    return NextResponse.json({ success: true, property: propertyData, token: accessToken, jurisdiction: "fortbend" });
  }
  ...
  if (sessionId) {
    try {
      ...
      if (!clientRef || !clientRef.startsWith("fortbend:")) {
        return NextResponse.json({ error: "Not a Fort Bend property" }, { status: 400 });
      }
      const acct = clientRef.replace("fortbend:", "");
      const propertyData = await getFortBendPropertyData(acct);
      if (!propertyData) {
        return NextResponse.json({ error: "Property not found" }, { status: 404 });
      }
      ...
      if (email) {
        try {
          ...
          const [quickStartPdf, evidencePdf, coverLetterPdf] = await Promise.all([
            ...
            generatePdf(generateFortBendEvidenceHtml(propertyData)),
            generatePdf(generateCoverLetterHtml(buildCoverLetterData(propertyData))),
          ]);
          await sendFortBendEmail(email, acct, quickStartPdf, evidencePdf, coverLetterPdf, propertyData, token);
        } catch (emailErr) {
          console.error("Email send error:", emailErr);
        }
      }
      return NextResponse.json({ success: true, property: propertyData, token, email: email || null, jurisdiction: "fortbend" });
    } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  const html = generateFortBendEvidenceHtml(propertyData);
