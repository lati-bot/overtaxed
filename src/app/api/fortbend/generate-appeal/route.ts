      ...
      <div class="steps">
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">FBCAD mails appraisal notices in <strong>late March to early April</strong>. Your notice will include a <strong>PIN number</strong> needed for online filing. Your property account: <strong style="font-family: monospace; background: #f0f0f0; padding: 1px 4px; border-radius: 3px;">${data.acct}</strong>.</div>
          </div>
        </div>
        <div class="step">
...
function generateAccessToken(sessionId: string, acct: string): string {
  const data = `${sessionId}:fortbend:${acct}:${Date.now()}`;
  const hash = crypto.createHash("sha256").update(data + process.env.STRIPE_SECRET_KEY).digest("hex");
  return `${Buffer.from(`fortbend:${acct}:${sessionId}`).toString("base64url")}.${hash.slice(0, 16)}`;
}
