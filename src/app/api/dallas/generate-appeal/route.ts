      ...
      <div class="steps">
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">DCAD mails appraisal notices in <strong>late March to early April</strong>. Your notice will show your property's appraised value for the current year. Your account number is <strong style="font-family: monospace; background: #f0f0f0; padding: 1px 4px; border-radius: 3px;">${data.acct}</strong>.</div>
          </div>
        </div>
        <div class="step">
  ...
  await getResend().emails.send({
    ...
    html: `
      <div style="background: #f7f6f3; padding: 32px 16px;">
        ...
        <p style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #666; margin-bottom: 24px; font-size: 15px;">Everything you need to protest your property tax for <strong>${data.address}</strong> with the Dallas Central Appraisal District.</p>
...
function generateAccessToken(sessionId: string, acct: string): string {
  const data = `${sessionId}:dallas:${acct}:${Date.now()}`;
  const hash = crypto.createHash("sha256").update(data + process.env.STRIPE_SECRET_KEY).digest("hex");
  return `${Buffer.from(`dallas:${acct}:${sessionId}`).toString("base64url")}.${hash.slice(0, 16)}`;
}
