...
import { DallasEvidenceData, generateDallasEvidenceHtml } from "@/lib/evidence-packet-dallas";
...
// ──────────────────────────────────────────────────────────────────
// Dallas Property Data Types
// ──────────────────────────────────────────────────────────────────

interface DallasComp {
  acct: string;
  address: string;
  sqft: number;
  assessedVal: number;
...
async function getDallasPropertyData(acct: string): Promise<DallasPropertyData | null> {
  try {
    ...
    // Map comps
    const comps: DallasComp[] = (prop.comps || []).map((c: any) => ({
      acct: c.acct || "",
      address: c.address || "",
      sqft: c.sqft || 0,
    ...
    console.error("[getDallasPropertyData] Error:", error);
    return null;
  }
}

// ──────────────────────────────────────────────────────────────────
// PDF HTML Generation — Dallas / Dallas County
// ──────────────────────────────────────────────────────────────────

function generateDallasPdfHtml(data: DallasPropertyData): string {
  const today = new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" });

  // Build property details rows (Dallas has richer data)
  ...
  const brief = generateDallasBrief(data);
  ...
  <div class="page">
    ...
    <div class="header">
      ...
      <div class="header-right">
        ...
        <div>Dallas County, Texas</div>
        <div>Protest Type: Uniform & Equal / Market Value</div>
        <div>Filing Body: DCAD / Appraisal Review Board</div>
      </div>
    </div>
    ...
    <!-- Property Details — Dallas has richer data -->
    ...
    <div class="section page-break">
      <div class="section-title">📋 How to Protest — Dallas County Step by Step</div>
      ...
      <div class="steps">
        ...
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">
              ...
              • Search using your <strong>Account Number</strong>: <strong style="font-family: monospace; background: #f0f0f0; padding: 1px 4px; border-radius: 3px;">${escapeHtml(data.acct)}</strong><br/>
        ...
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">
              After filing, DCAD may offer a <strong>settlement</strong> without a hearing.<br/>
...
function generateDallasBrief(data: DallasPropertyData): string {
  // Include richer property description for Dallas
...
function buildQuickStartData(data: DallasPropertyData): QuickStartData {
  return {
    ...
    portalScreenshotDataUri: getPortalScreenshotDataUri("Dallas") ?? undefined,
  };
}

function buildCoverLetterData(data: DallasPropertyData): CoverLetterData {
  return {
    ...
    state: "TX",
    filingBody: "DCAD",
    filingBodyFull: "Dallas Central Appraisal District",
    currentAssessment: data.currentAssessment,
    fairAssessment: data.fairAssessment,
    overAssessedPct: data.overAssessedPct,
    compCount: data.comps.length,
...
async function sendDallasEmail(
  email: string,
  acct: string,
  quickStartPdf: Buffer,
  evidencePdf: Buffer,
  coverLetterPdf: Buffer,
  data: DallasPropertyData,
  accessToken: string
): Promise<void> {
      ...
      <div style="background: #f7f6f3; padding: 32px 16px;">
        <div style="max-width: 600px; margin: 0 auto;">
          ...
          <div style="background: #ffffff; padding: 28px 24px; border: 1px solid #e2e2e0; border-top: none; border-radius: 0 0 12px 12px;">
            ...
            <p style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #666; margin-bottom: 24px; font-size: 15px;">Everything you need to protest your property tax for <strong>${escapeHtml(data.address)}</strong> with the Dallas Central Appraisal District.</p>
            ...
            <div style="background: #e8f4f0; border: 2px solid #1a6b5a; border-radius: 10px; padding: 18px 20px; margin-bottom: 24px;">
              ...
              <p style="margin: 0; font-size: 13px; color: #1a6b5a; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">Current: $${data.currentAssessment.toLocaleString()} → Fair: $${data.fairAssessment.toLocaleString()} (${data.overAssessedPct}% over-appraised)</p>
            </div>
...
// ──────────────────────────────────────────────────────────────────
// Token handling — dallas:acct:sessionId format
// ──────────────────────────────────────────────────────────────────

function generateAccessToken(sessionId: string, acct: string): string {
  return _genToken(`dallas:${acct}:${sessionId}`);
}
...
function verifyAccessToken(token: string): { acct: string; sessionId: string } | null {
  ...
  if (parts.length < 3 || parts[0] !== "dallas") return null;
...
export async function GET(request: NextRequest) {
  ...
  if (accessToken) {
    ...
    const propertyData = await getDallasPropertyData(tokenData.acct);
    if (!propertyData) {
      return NextResponse.json({ error: "Property not found" }, { status: 404 });
    }
    return NextResponse.json({ success: true, property: propertyData, token: accessToken, jurisdiction: "dallas" });
  }
  ...
  if (sessionId) {
    try {
      ...
      if (!clientRef || !clientRef.startsWith("dallas:")) {
        return NextResponse.json({ error: "Not a Dallas property" }, { status: 400 });
      }
      const acct = clientRef.replace("dallas:", "");
      const propertyData = await getDallasPropertyData(acct);
      if (!propertyData) {
        return NextResponse.json({ error: "Property not found" }, { status: 404 });
      }
      ...
      if (email) {
        try {
          ...
          const [quickStartPdf, evidencePdf, coverLetterPdf] = await Promise.all([
            ...
            generatePdf(generateDallasEvidenceHtml(propertyData)),
            generatePdf(generateCoverLetterHtml(buildCoverLetterData(propertyData))),
          ]);
          console.log(`[Dallas] PDFs generated: quick-start ${quickStartPdf.length} bytes, evidence ${evidencePdf.length} bytes`);
          await sendDallasEmail(email, acct, quickStartPdf, evidencePdf, coverLetterPdf, propertyData, token);
...
export async function POST(request: NextRequest) {
  ...
  const html = generateDallasEvidenceHtml(propertyData);
