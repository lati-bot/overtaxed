  ...
  <div class="page">
    ...
    <div class="summary-row">
      ...
      <div class="summary-card success">
        ...
        <div class="summary-detail">Median of ${data.comps.length} comparable properties in neighborhood ${data.neighborhoodCode}</div>
      </div>
      <div class="summary-card success">
    ...
    <div class="breakdown-row">
      <div class="breakdown-card">
        ...
        <table class="breakdown-table">
          <tr><td>Account Number</td><td>${data.acct}</td></tr>
    ...
    <div class="section">
      ...
      <p style="font-size: 10px; color: #555; margin-bottom: 10px;">The following ${data.comps.length} comparable properties are located within the same appraisal neighborhood (${data.neighborhoodCode}) and demonstrate that the subject property is appraised at a value significantly above comparable properties.</p>
      ...
      <table class="comps-table">
        ...
        <tbody>
          <tr class="subject-row">
            ...
            <td class="right" style="color: #dc2626; font-weight: 700;">$${data.currentAssessment.toLocaleString()}</td>
          </tr>
          ${compsHtml}
        </tbody>
      </table>
      ...
      <div class="comps-summary">
        <div class="comps-summary-item">
          ...
          <div class="value" style="color: #dc2626;">$${data.currentAssessment.toLocaleString()}</div>
        </div>
        <div class="comps-summary-item">
      ...
      <div class="steps">
        <div class="step">
          ...
          <div class="step-content">
            ...
            <div class="step-desc">RCAD mails appraisal notices in <strong>late March to early April</strong>. Your notice will include information needed for online filing. Your property account: <strong style="font-family: monospace; background: #f0f0f0; padding: 1px 4px; border-radius: 3px;">${data.acct}</strong>.</div>
          </div>
        </div>
        <div class="step">
    ...
    <div class="section">
      ...
      <div style="background: #f8f9fa; border: 2px dashed #cbd5e1; border-radius: 8px; padding: 16px 18px; font-size: 10.5px; line-height: 1.7; color: #333; font-family: 'Georgia', serif;">
        ...
        <p style="margin-bottom: 8px;">"I've identified ${data.comps.length} comparable properties in the same neighborhood (${data.neighborhoodCode}) with similar characteristics. The median appraised value of these comparable properties is <strong>$${data.compMedianValue.toLocaleString()}</strong> — my property is appraised <strong>${data.overAssessedPct}% higher</strong> than comparable properties."</p>
        <p style="margin-bottom: 8px;">"Under the Texas Tax Code Section 42.26, property must be appraised equally and uniformly. I am requesting my appraised value be reduced to <strong>$${data.fairAssessment.toLocaleString()}</strong>, which reflects the median appraised value of comparable properties in my neighborhood."</p>
...
function generateRockwallBrief(data: RockwallPropertyData): string {
  return `
    <p>The subject property at <strong>${data.address}</strong> (RCAD Account: ${data.acct}), located in appraisal neighborhood ${data.neighborhoodCode}, is currently appraised at <strong>$${data.currentAssessment.toLocaleString()}</strong>.</p>
    
    <p>An analysis of ${data.comps.length} comparable properties within the same appraisal neighborhood demonstrates that the subject property is appraised at a value <strong>${data.overAssessedPct}% above the comparable median</strong> of $${data.compMedianValue.toLocaleString()}. This constitutes an unequal appraisal in violation of <strong>Texas Tax Code Section 42.26</strong>, which requires that the appraised value of property not exceed the median appraised value of comparable properties in the same area.</p>
...
async function sendRockwallEmail(
  ...
  await getResend().emails.send({
    ...
    to: email,
    subject: `Property Tax Protest Package — ${data.address} — Rockwall County`,
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; color: #1a1a2e;">
        ...
        <p style="color: #666; margin-bottom: 24px; font-size: 15px;">Everything you need to protest your property tax for <strong>${data.address}</strong> with the Rockwall Central Appraisal District.</p>
...
function generateAccessToken(sessionId: string, acct: string): string {
  const data = `${sessionId}:rockwall:${acct}:${Date.now()}`;
  const hash = crypto.createHash("sha256").update(data + process.env.STRIPE_SECRET_KEY).digest("hex");
  return `${Buffer.from(`rockwall:${acct}:${sessionId}`).toString("base64url")}.${hash.slice(0, 16)}`;
}
