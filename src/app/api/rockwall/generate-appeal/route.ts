...
function buildCoverLetterData(data: RockwallPropertyData): CoverLetterData {
  return {
    ...
    state: "TX",
    filingBody: "RCAD",
    filingBodyFull: "Rockwall Central Appraisal District",
    currentAssessment: data.currentAssessment,
    fairAssessment: data.fairAssessment,
    overAssessedPct: data.overAssessedPct,
    compCount: data.comps.length,
    ...
    try {
      ...
      if (email) {
        try {
          ...
          await sendRockwallEmail(email, acct, quickStartPdf, evidencePdf, coverLetterPdf, propertyData, token);
        } catch (emailErr) {
