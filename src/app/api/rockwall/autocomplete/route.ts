...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Remove common Rockwall County city suffixes
    cleaned = cleaned.replace(/,?\s*(ROCKWALL|HEATH|FATE|ROYSE CITY|MCLENDON CHISHOLM|MCLENDON-CHISHOLM|WYLIE|ROWLETT)\b.*$/i, "");
    cleaned = cleaned.replace(/,?\s*TX\s*\d{0,5}\s*$/i, "").trim();

    // Search by address using STARTSWITH for fast prefix matching
    // No sqft filter — Rockwall data doesn't have sqft. Filter by current_assessment instead.
    const { resources } = await container.items.query({
      query: `SELECT TOP 8 c.id, c.acct, c.address, c.city, c.neighborhood_code, c.current_assessment, c.estimated_savings
              FROM c 
    ...
    console.error("[rockwall/autocomplete] Error:", error);
